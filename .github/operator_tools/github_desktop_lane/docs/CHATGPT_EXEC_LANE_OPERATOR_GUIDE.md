...
3. ChatGPT stages a request JSON under `.github/chatgpt_staging/exec_requests/`.
4. GitHub Actions runs the command on a Windows runner.
5. The workflow writes a status report under `.github/chatgpt_staging/status_reports/chatgpt-exec/`.
...
1. Build a ChatGPT apply-in ZIP with an `apply_manifest.json` that creates exactly one reviewed request file under `.github/chatgpt_staging/exec_requests/<request_id>.json`.
2. Stage the payload as `.github/chatgpt_staging/in/<apply_request_id>/payload.zip.b64`.
3. Let `chatgpt-apply-in` run and verify its report under `.github/chatgpt_staging/status_reports/chatgpt-apply-in/<apply_request_id>/workflow_report.md`.
...
8. Verify the exec report under `.github/chatgpt_staging/status_reports/chatgpt-exec/<request_id>/workflow_report.md`.
...
- GitHub connector action that triggered exec: `update_file` on `.github/chatgpt_staging/exec_requests/exec-20260503-airtable-schema-hashfix-002.json`
...
Use `.github/operator_tools/github_desktop_lane/manifests/chatgpt_exec_request.sample.json` as the starting shape.
