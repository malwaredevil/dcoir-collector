...
def request_dir_from_path(repo: pathlib.Path, request_path: pathlib.Path, request_id: str) -> pathlib.Path:
    try:
        rel = request_path.resolve().relative_to((repo / ".github/ops/requests/apply_patch").resolve())
    except ValueError as exc:
        raise RequestError("request file must live under .github/ops/requests/apply_patch/<request_id>/request.json") from exc
    if rel.parts != (request_id, "request.json"):
        raise RequestError("request file path must be .github/ops/requests/apply_patch/<request_id>/request.json")
...
def load_request(repo: pathlib.Path, request_path: pathlib.Path, default_branch: str) -> PatchRequest:
    ...
    expected_prefix = f".github/ops/requests/apply_patch/{request_id}/"
