...
def resolve(args) -> int:
    ...
    if args.caller_event_name == "workflow_dispatch" and args.input_request_path:
        ...
        if not safe_request_path(request_path):
            raise SystemExit("request_path must match .github/ops/requests/apply_patch/<request_id>/request.json")
        ...
        if not pending_records:
            print("No unmarked apply-patch requests were found under .github/ops/requests/apply_patch within the scan window.")
    ...
    if not safe_request_path(request_path):
        raise SystemExit(f"resolved request_path must match .github/ops/requests/apply_patch/<request_id>/request.json: {request_path}")
