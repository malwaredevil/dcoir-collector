...
- [ ] Before every `/dcoir-review` invocation or rerun, the exact proposed command was shown to the operator and explicitly approved in the current session; no approval means no DCOIR Review request
...
- [ ] No `/dcoir-review` command, including `deep`, `diff`, `debug`, or any other variant, was posted or confirmed without operator approval of the exact proposed command in the current session
- [ ] Every DCOIR Review rerun received fresh current-session operator approval; a prior approval was not reused
...
- [ ] Every `/dcoir-review` invocation used for readiness evidence was operator-approved as an exact current-session request, including each rerun
