...
5. applyable GitHub repairs are stricter than ordinary findings. Detector-authored replacement text is stripped. Only verifier-supported findings inside the bounded repair budget enter the v36/v38 repair-set pipeline:
   ...
   - repair-author confidence is retained as diagnostic evidence but is advisory rather than a semantic acceptance gate; missing author confidence is recorded as `0.0`, never invented;
   - a missing or blank per-edit `purpose` may be filled only with deterministic explanatory metadata derived from the verified finding; DCOIR must not synthesize or forgive missing path, line range, original block, replacement block, or other repair semantics;
   ...
   - an independent frontier model from a different model family critiques the **whole repair set**, including completeness and whether each edit is necessary for the verified root cause; critic acceptance is the hard semantic repair gate and requires confidence of at least `0.95`;
...
- **Ongoing live-watch:** closing a hardening issue does not freeze the reviewer. A confirmed high-signal miss, serious false positive, lost-finding/publication seam, unsafe or knowingly incomplete repair, changed-file coverage omission, or material precision/recall regression must be treated as evidence for a new bounded hardening round and a generalized permanent regression case. Improvements should remain generic rather than hard-coded to the PR that exposed the weakness.
...
Issue #456 adds newer controlled evidence layers. Blind DCOIR run `33372955952` on unchanged PR #448 independently rediscovered the same rejected-proposition polarity defect previously identified by GitHub Copilot Balanced, demonstrating materially improved semantic recall without hard-coding the PR answer. The v36 active-chain regression then proved the production synthesis wrapper can carry a synthetic verifier-supported finding through a two-line repair set, fixed independent frontier repair critic, exact-head validation, and ranged GitHub suggestion payload without network-dependent model calls.

The later v37 blind run `33384820835` completed successfully against unchanged PR #448 head `6355adf7b560736f5c202e251070b52516c3a1e6` and provides the stronger live recall/publication evidence. All 31 changed files received primary review; the primary detector produced 24 structured findings; the Sol Pro challenger completed; 27 hypotheses entered Opus adjudication; five were retained; normalization produced three verifier candidates; and all three were independently model-verified and published. One published finding was the exact rejected-proposition `explicit_no_mix` defect at changed line 224, proving the finding survived detector → challenger → semantic adjudicator → v37 result-shape normalization → verifier → GitHub publication. The same run also exposed a repair-stage contract weakness: all three repair authors supplied concrete exact edits, but the v36 pipeline rejected them before independent criticism because the non-semantic `purpose` field was omitted, while author confidence was absent, 0.85, and 0.78. v38 hardens that seam by making author confidence advisory, normalizing only explanatory metadata, and raising the independent critic hard acceptance threshold to 0.95.

These controls do not by themselves satisfy the rolling naturalistic precision or repair-safety sample requirements. A live multi-line or multi-block coordinated suggestion is still required before coordinated repair capability is treated as production-proven. In particular, #293 remains blocked and autonomous branch writes remain unauthorized.
