def detect_python_file_write_path_sentinels(diff: str) -> list[hardened.RiskSentinel]:
    ...
    for diff_line in iter_python_diff_lines_with_context(diff):
        ...
        _prune_assigned_int_bindings(assigned_int_bindings, active_scope_ids)
        ...
        if dynamic_target:
            ...
            if _line_opens_conditional_block(diff_line.text):
        ...
        current_int_bindings = visible_int_bindings()
        active_shadowed_names = shadowed_name_roots_by_line.get(diff_line.path, {}).get(diff_line.line)
        write_target = python_file_write_target(
            diff_line.text,
            path_constructor_names,
            os_module_names,
