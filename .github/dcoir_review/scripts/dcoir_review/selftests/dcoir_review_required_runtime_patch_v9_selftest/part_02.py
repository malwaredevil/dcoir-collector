...
def test_powershell_validation_is_quote_safe() -> None:
    ...
    dollar_path_validation = v9._validation_for_key(v9.v5.PS_ENV_TOKEN, ".github/chatgpt_staging/$bad/path.ps1", 15)
    assert "$bad" in dollar_path_validation
    assert '$p = ".github/chatgpt_staging/$bad/path.ps1"' not in dollar_path_validation
    assert ".github/chatgpt_staging/$bad/path.ps1" in dollar_path_validation
