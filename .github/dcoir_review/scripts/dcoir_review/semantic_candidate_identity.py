"""Stable semantic-candidate identity and provenance protection for DCOIR Review.
...
input. This owner preserves ordinary semantic identity across ranking and final
selection unless explicit or anchored risk provenance supports deterministic
classification. Publication verification remains authoritative and unchanged.

Historical v51 field names, debug artifact paths, schemas, and the contract
version remain compatibility/provenance data because downstream recovery and
durable diagnostics consume them; they no longer represent runtime ownership.
"""
...
def _original_postable_key() -> Any:
    ...
    if not callable(current):
        raise RuntimeError("DCOIR semantic-candidate identity could not locate the v16 postable-key helper")
...
def _patch_ranker(module: Any) -> None:
    ...
    if not callable(original):
        raise RuntimeError("DCOIR semantic-candidate identity could not locate rank_findings_for_required_budget")
