...
This terminal overlay is intentionally behavior-preserving for semantic review:
it does not change prompts, model/provider selection, routing, escalation, verifier
policy, or repair policy.  It creates one deterministic review-context package,
records conservative reuse telemetry, and makes that package available to later
Architecture-B stages.
"""
...
def _valid_blob_sha(value: str) -> bool:
    ...
    return len(candidate) in {40, 64} and all(character in "0123456789abcdef" for character in candidate)


def _file_record(item: dict[str, Any], *, schema_sha256: str, config_sha256: str, dependency_sha256: str) -> dict[str, Any]:
    path = str(item.get("filename", "") or "").replace("\\", "/").strip()
    status = str(item.get("status", "") or "").strip().lower()
    blob_sha = str(item.get("sha", "") or "").strip().lower()
    content_identity_available = status in {"removed", "deleted"} or _valid_blob_sha(blob_sha)
    content_identity = (
        f"deleted:{path}"
    ...
    base_record = {
        "path": path,
        "previous_path": str(item.get("previous_filename", "") or "").replace("\\", "/").strip(),
        "status": status,
        ...
        "patch_sha256": _text_digest(item.get("patch", "")) if item.get("patch") else "",
    }
    record = dict(base_record)
    record["surface_fingerprint"] = _digest(base_record)
    if content_identity_available:
        record["prospective_reuse_key"] = _digest(
            {
...
def _line_index_digest(line_index: dict[tuple[str, int], int]) -> str:
    normalized_rows = [
        ...
        for (path, line), position in sorted(line_index.items(), key=lambda pair: (str(pair[0][0]), int(pair[0][1])))
    ]
    return _digest(normalized_rows)
...
def build_semantic_review_ledger(
    ...
    """Build one canonical, deterministic package describing semantic review input."""
    ...
    risk_digest_fn = getattr(getattr(module, "hardened", None), "risk_sentinel_digest", None)
    risk_digest = str(risk_digest_fn(risk_sentinels) if callable(risk_digest_fn) and risk_sentinels else "")
    pr_body = str(pr.get("body", "") or "") if isinstance(pr, dict) else ""
    pr_context = {
        "number": int(pr.get("number", 0) or 0) if isinstance(pr, dict) else 0,
        "title_sha256": _text_digest(pr.get("title", "") if isinstance(pr, dict) else ""),
        "body_sha256": _text_digest(pr_body),
        "base_ref": str(pr.get("base", {}).get("ref", "") or "") if isinstance(pr.get("base", {}), dict) else "",
        "base_sha": str(pr.get("base", {}).get("sha", "") or "").lower() if isinstance(pr.get("base", {}), dict) else "",
        "head_ref": str(pr.get("head", {}).get("ref", "") or "") if isinstance(pr.get("head", {}), dict) else "",
    ...
    review_surface = {
        ...
        "current_head_sha": str(scope.get("current_head_sha", "") or pr_context["head_sha"]),
        "prior_reviewed_base_sha": str(scope.get("prior_reviewed_base_sha", "") or ""),
        "current_base_sha": str(scope.get("current_base_sha", "") or pr_context["base_sha"]),
    ...
    context_inputs = {
        ...
        "context_summary_sha256": _text_digest(context_summary),
        "context_summary_chars": len(str(context_summary or "")),
    ...
    context_fingerprint = _digest(fingerprint_inputs)
    missing_content_identity = [
        item["path"] for item in file_records if not bool(item.get("content_identity_available"))
    ]
    ledger = {
        "semantic_ledger_contract": SEMANTIC_LEDGER_CONTRACT,
        "architecture_contract": ARCHITECTURE_CONTRACT,
...
def _ledger_marker(ledger: dict[str, Any]) -> str:
    telemetry = ledger.get("telemetry", {}) if isinstance(ledger.get("telemetry"), dict) else {}
...
def apply_pareto_context_module(module: Any) -> None:
    ...
    def write_debug_json_artifact_safely(config: Any, relative_path: str, value: Any) -> None:
        if relative_path == "metadata/review-context.json" and isinstance(value, dict) and _LAST_LEDGER:
            enriched = dict(value)
            telemetry = _LAST_LEDGER.get("telemetry", {})
            enriched.update(
                {
                    "semantic_ledger_contract": SEMANTIC_LEDGER_CONTRACT,
                    "semantic_context_fingerprint": str(_LAST_LEDGER.get("context_fingerprint", "") or ""),
                    "semantic_reviewed_file_count": int(telemetry.get("reviewed_file_count", 0) or 0),
                    "semantic_reused_file_count": int(telemetry.get("reused_file_count", 0) or 0),
                    "semantic_recomputed_file_count": int(telemetry.get("recomputed_file_count", 0) or 0),
                    "semantic_dependency_expanded_file_count": int(telemetry.get("dependency_expanded_file_count", 0) or 0),
                    "semantic_invalidation_reason": str(telemetry.get("invalidation_reason", "") or ""),
                }
            )
            value = enriched
    ...
    def openrouter_review_with_hybrid_first_pass(
        ...
        module.hardened.write_debug_json_artifact_safely(
            config, "metadata/semantic-review-ledger.json", copy.deepcopy(ledger)
        )
        try:
