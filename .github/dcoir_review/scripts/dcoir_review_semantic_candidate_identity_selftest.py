#!/usr/bin/env python3
"""Offline regressions for stable semantic-candidate identity ownership."""
...
def test_free_text_risk_inference_cannot_rewrite_semantic_candidate(review) -> None:
    ...
    inferred = candidate_identity._legacy_normalized_key(candidate)
    assert inferred[2], inferred
    assert candidate_identity._risk_provenance(candidate, inferred) == ""
    ...
    assert selected.get(candidate_identity.CANDIDATE_ID_FIELD)
    semantic_key = selected.get(candidate_identity.SEMANTIC_KEY_FIELD)
    assert isinstance(semantic_key, list) and len(semantic_key) == 3, selected
    assert str(semantic_key[2]).startswith(candidate_identity.SEMANTIC_KIND_PREFIX)
...
def test_same_site_semantic_candidates_keep_distinct_identity(review) -> None:
    ...
    ids = [str(item.get(candidate_identity.CANDIDATE_ID_FIELD, "")) for item in ranked]
    keys = [tuple(item.get(candidate_identity.SEMANTIC_KEY_FIELD, [])) for item in ranked]
...
def test_required_sentinel_same_site_supplements_not_replaces(review) -> None:
    ...
    candidate_id = str(ranked[0][candidate_identity.CANDIDATE_ID_FIELD])
    ...
    semantic = [item for item in final if item.get(candidate_identity.CANDIDATE_ID_FIELD) == candidate_id]
    assert len(semantic) == 1, final
    assert semantic[0]["title"] == candidate["title"]
    assert semantic[0]["body"] == candidate["body"]
    assert semantic[0][candidate_identity.SEMANTIC_KEY_FIELD] == ranked[0][candidate_identity.SEMANTIC_KEY_FIELD]

    required_coverage = candidate_identity_hooks._sentinel_coverage([sentinel])
    required = [
        item
        for item in final
        if item.get(candidate_identity.CANDIDATE_ID_FIELD) != candidate_id
        and candidate_identity_hooks._is_required_selection(item, required_coverage)
    ]
    assert len(required) == 1, final

    manifest = captured[candidate_identity.FINAL_SELECTION_ARTIFACT_PATH]
    disposition = next(
...
def test_explicit_risk_provenance_keeps_required_risk_behavior(review) -> None:
    ...
    inferred = candidate_identity._legacy_normalized_key(candidate)
    assert inferred[2], inferred
    assert candidate_identity._risk_provenance(candidate, inferred) in {
    ...
    ranked = review.rank_findings_for_required_budget([candidate], cfg)
    assert len(ranked) == 1, ranked
    assert ranked[0].get(candidate_identity.CANDIDATE_ID_FIELD)
    assert not ranked[0].get(candidate_identity.SEMANTIC_KEY_FIELD)
    ...
    assert candidate_identity_hooks._is_required_selection(final[0], candidate_identity_hooks._sentinel_coverage([sentinel]))
...
def test_unsupported_deterministic_hypothesis_can_still_be_suppressed(review) -> None:
    ...
    assert deterministic.get(candidate_identity.CANDIDATE_ID_FIELD)
    ...
    manifest = captured[candidate_identity.VERIFIER_ARTIFACT_PATH]
    assert manifest["candidate_count"] == 2
    assert {item["candidate_id"] for item in manifest["candidates"]} == {
        semantic[candidate_identity.CANDIDATE_ID_FIELD],
        deterministic[candidate_identity.CANDIDATE_ID_FIELD],
    }
...
def test_debug_manifest_records_candidate_provenance(review) -> None:
    ...
    try:
        ...
        review.hardened.write_debug_json_artifact_safely = original_writer
    assert len(ranked) == 1
    manifest = captured[candidate_identity.SELECTION_ARTIFACT_PATH]
    ...
    assert record["candidate_id"] == ranked[0][candidate_identity.CANDIDATE_ID_FIELD]
    ...
    assert snapshot["semantic_key"] == ranked[0][candidate_identity.SEMANTIC_KEY_FIELD]
...
def test_production_registration_and_config(review) -> None:
    ...
    assert "dcoir_review_semantic_candidate_identity_selftest.py" in production
...
def main() -> None:
    ...
    print("dcoir_review_semantic_candidate_identity_selftest passed")
