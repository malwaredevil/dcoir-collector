#!/usr/bin/env python3
"""Semantic fixture guards for the historical clean-precision v4 corpus.

These checks preserve the exact v4 benchmark construction for reproducibility;
they are not the current acceptance bar. They inspect the right-side/current
representation of each relevant changed file so prior paid evidence remains
replayable after later corpus hardening.
"""
...
def main() -> None:
    cases = {str(case["id"]): case for case in precision.load_v4_cases()}
    assert len(cases) == 10
    ...
    print("dcoir_review_pr_precision_fixture_v4_selftest passed: historical v4 fixture invariants remain reproducible; current acceptance uses v5")
