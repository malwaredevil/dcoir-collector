...
- the status surface is an explicit four-module composition: `dcoir_review/status.py` owns single-comment discovery/update and race reconciliation, `dcoir_review/status_overview.py` owns compact operator-facing rendering, `dcoir_review/status_overview_support.py` owns hidden rerun provenance encoding plus safe inline HTML rendering helpers and a complete opaque finding-identity index kept separate from bounded display details, and `dcoir_review/status_snapshot.py` owns normalized changed-file/finding snapshots; normal runs progress through concise queued/running/terminal states and finish as one Copilot-style overview on the same bot-authored comment, while debug runs retain detailed progress and diagnostics;
...
The stable status contract is covered by `dcoir_review_status_comment_selftest.py`, including rerun reuse and same-comment identity, exact-head/formal-review provenance, inline finding links and severity summaries, changed-head resolved findings, same-head previously-missed findings, compact normal failure output, verbose debug diagnostics, spoofed-user marker rejection, and observational write failures. `dcoir_review_runtime_module_loader_selftest.py` also owns the connector-safe size and direct-import registration contract for all four status modules. Exact-head status-cutover validation passed in ChatGPT Exec run `34695700055` at source head `efabcaec43676951a596afed06c601dc8486d840`; current-head CodeQL also passed at that source head.
