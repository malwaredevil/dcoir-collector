...
- the status surface is an explicit three-module composition: `dcoir_review/status.py` owns single-comment discovery/update and race reconciliation, `dcoir_review/status_overview.py` owns compact operator-facing rendering plus hidden rerun provenance, and `dcoir_review/status_snapshot.py` owns normalized changed-file/finding snapshots; normal runs progress through concise queued/running/terminal states and finish as one Copilot-style overview on the same bot-authored comment, while debug runs retain detailed progress and diagnostics;
