...
$newRequestId = 'issue550-pr553-v39-semantic-adjudication-confidence-validation-001'
$oldMarker = 'ISSUE550_PR553_V58_PROVIDER_TRANSPORT_RETRY_VALIDATION_001_PASS'
$newMarker = 'ISSUE550_PR553_V39_SEMANTIC_ADJUDICATION_CONFIDENCE_VALIDATION_001_PASS'
$text = Get-Content -Raw -LiteralPath $source

$text = Replace-Required $text '4fa7fbb9ec9d287e6a084c2d4ab81212e83bd787' '3e48c4f2ab8b4f9a76f540b7f18cd59065d06812' 'exact head'
$text = Replace-Required $text '7dddb386b3ca829babc4d0fe59cb83ec0f872bd0' 'c63825d9b6d1a0f4470b2971c575dfb67ebd7607' 'exact tree'
...
$text = Replace-Required $text 'issue550-pr553-v58-validation-' 'issue550-pr553-v39-validation-' 'isolated worktree prefix'
...
$text = Replace-Required $text '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v39_selftest.py' '.github/dcoir_review/scripts/dcoir_review_semantic_adjudication_confidence_selftest.py' 'stable v39 selftest path'

$oldRetiredV39 = "'dcoir_review_required_runtime_patch_v37','dcoir_review_required_runtime_patch_v47','dcoir_review_required_runtime_patch_v55','dcoir_review_required_runtime_patch_v58')"
$newRetiredV39 = "'dcoir_review_required_runtime_patch_v37','dcoir_review_required_runtime_patch_v39','dcoir_review_required_runtime_patch_v47','dcoir_review_required_runtime_patch_v55','dcoir_review_required_runtime_patch_v58')"
$text = Replace-Required $text $oldRetiredV39 $newRetiredV39 'retired v39 production module set'

$oldStableV39 = "'dcoir_review.per_file_routing','dcoir_review.provider_transport_retry','dcoir_review.semantic_adjudication_normalization','dcoir_review.semantic_adjudication_recovery')"
$newStableV39 = "'dcoir_review.per_file_routing','dcoir_review.provider_transport_retry','dcoir_review.semantic_adjudication_normalization','dcoir_review.semantic_adjudication_confidence','dcoir_review.semantic_adjudication_recovery')"
$text = Replace-Required $text $oldStableV39 $newStableV39 'stable v39 responsibility owner set'

$confidenceCompositionAnchor = @'
        $v36Index = [Array]::IndexOf($sequence, 'dcoir_review_required_runtime_patch_v36')
        $normalizationIndex = [Array]::IndexOf($sequence, 'dcoir_review.semantic_adjudication_normalization')
        $v38Index = [Array]::IndexOf($sequence, 'dcoir_review_required_runtime_patch_v38')
        if ($v36Index -lt 0 -or $normalizationIndex -ne ($v36Index + 1) -or $v38Index -ne ($normalizationIndex + 1)) { throw "Semantic adjudication normalization composition mismatch: v36=$v36Index normalization=$normalizationIndex v38=$v38Index" }
...
'@
$text = Replace-Required $text $confidenceCompositionAnchor ($confidenceCompositionAnchor + $confidenceComposition) 'stable semantic confidence composition assertion'
$confidenceSourceAnchor = @'
        $normalizationBytes = (Get-Item -LiteralPath $normalizationPath).Length
        if ($normalizationBytes -gt 15000) { throw "Stable semantic normalization owner exceeds connector-safe size: $normalizationBytes" }
'@
$confidenceSourceChecks = @'
...
'@
$text = Replace-Required $text $confidenceSourceAnchor ($confidenceSourceAnchor + $confidenceSourceChecks) 'stable semantic confidence source assertions'
$v44ConfidenceAnchor = @'
        if ($v44ExecutionText -match 'import dcoir_review_required_runtime_patch_v37') { throw 'v44 execution still imports historical v37 owner' }
'@
$v44ConfidenceChecks = @'
        if ($v44ExecutionText -notmatch 'from dcoir_review import semantic_adjudication_confidence as confidence') { throw 'v44 execution does not import stable semantic confidence owner' }
...
'@
$text = Replace-Required $text $v44ConfidenceAnchor ($v44ConfidenceAnchor + $v44ConfidenceChecks) 'v44 stable confidence dependency assertions'

$entryConfidenceAnchor = @'
        if ($entrypointText -notmatch '["'']dcoir_review\.semantic_adjudication_recovery["'']') { throw 'Entrypoint does not compose stable semantic adjudication recovery owner' }
'@
$entryConfidenceChecks = @'
        if ($entrypointText -match '["'']dcoir_review_required_runtime_patch_v39["'']') { throw 'Entrypoint still composes historical v39 owner' }
...
'@
$text = Replace-Required $text $entryConfidenceAnchor ($entryConfidenceAnchor + $entryConfidenceChecks) 'entrypoint retired/stable v39 ownership assertions'
...
'@
$text = Replace-Required $text $v39RefAnchor ($v39RefAnchor + $v39RefChecks) 'active-source v39 provenance scan'
$provConfidenceAnchor = @'
        if ($provSequence -notcontains 'dcoir_review.semantic_adjudication_recovery') { throw 'Stable semantic adjudication recovery owner missing from runtime provenance' }
'@
$provConfidenceChecks = @'
        if ($provSequence -contains 'dcoir_review_required_runtime_patch_v39') { throw 'Historical v39 appears in runtime provenance' }
...
'@
$text = Replace-Required $text $provConfidenceAnchor ($provConfidenceAnchor + $provConfidenceChecks) 'runtime provenance v39 retirement assertions'

$text = Replace-Required $text "'retired=v15,v19,v21,v22,v23,v24,v25,v26,v27,v28,v29,v37,v47,v55,v58'" "'retired=v15,v19,v21,v22,v23,v24,v25,v26,v27,v28,v29,v37,v39,v47,v55,v58'" 'summary retired v39 set'

$summaryCompositionAnchor = @'
            'composition_normalization=v36->semantic_adjudication_normalization->v38',
'@
$summaryCompositionConfidence = @'
            'composition_confidence=v38->semantic_adjudication_confidence->v31',
'@
$text = Replace-Required $text $summaryCompositionAnchor ($summaryCompositionAnchor + $summaryCompositionConfidence) 'summary stable confidence composition'

$summaryOwnerAnchor = @'
            'semantic_adjudication_normalization_owner=dcoir_review.semantic_adjudication_normalization',
'@
$summaryOwnerConfidence = @'
            'semantic_adjudication_confidence_owner=dcoir_review.semantic_adjudication_confidence',
'@
$text = Replace-Required $text $summaryOwnerAnchor ($summaryOwnerAnchor + $summaryOwnerConfidence) 'summary stable confidence owner'

$summaryBytesAnchor = @'
            "semantic_adjudication_normalization_bytes=$normalizationBytes",
'@
$summaryBytesConfidence = @'
            "semantic_adjudication_confidence_bytes=$confidenceBytes",
'@
$text = Replace-Required $text $summaryBytesAnchor ($summaryBytesAnchor + $summaryBytesConfidence) 'summary stable confidence owner size'
