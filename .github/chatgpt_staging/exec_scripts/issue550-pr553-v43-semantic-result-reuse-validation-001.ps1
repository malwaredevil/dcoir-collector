...
$requestId = 'issue550-pr553-v43-semantic-result-reuse-validation-001'
$requestRel = ".github/chatgpt_staging/exec_requests/$requestId.json"
$requestPath = Join-Path $repo $requestRel
$terminalMarker = 'ISSUE550_PR553_V43_SEMANTIC_RESULT_REUSE_VALIDATION_001_PASS'
...
try {
    ...
    try {
        ...
        foreach ($consumer in @('.github/dcoir_review/scripts/dcoir_review/semantic_review_ledger_fingerprints.py','.github/dcoir_review/scripts/dcoir_review/semantic_review_ledger_builder.py','.github/dcoir_review/scripts/dcoir_review/semantic_result_reuse.py','.github/dcoir_review/scripts/dcoir_review/semantic_result_reuse_support.py','.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v46_context.py','.github/dcoir_review/scripts/dcoir_review/verified_finding_gate_prior.py')) { $consumerText=Get-Content -Raw -LiteralPath $consumer; if ($consumerText -match 'dcoir_review_required_runtime_patch_v41') { throw "Downstream consumer retains historical v41 dependency: $consumer" } }
        ...
        $v42Index = [Array]::IndexOf($sequence, 'dcoir_review.semantic_review_ledger')
        $v43Index = [Array]::IndexOf($sequence, 'dcoir_review.semantic_result_reuse')
        ...
        $historicalV42 = @(
            ...
            '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v42_contract.py',
            '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v42_fingerprints.py',
            '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v42_hooks.py',
            '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v42_ledger.py',
            '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v42_selftest.py'
        )
        foreach ($path in $historicalV42) { if (Test-Path -LiteralPath $path) { throw "Retired historical v42 owner still exists: $path" } }
        $ledgerPath = '.github/dcoir_review/scripts/dcoir_review/semantic_review_ledger.py'
        $ledgerBuilderPath = '.github/dcoir_review/scripts/dcoir_review/semantic_review_ledger_builder.py'
        $ledgerContractPath = '.github/dcoir_review/scripts/dcoir_review/semantic_review_ledger_contract.py'
        $ledgerFingerprintsPath = '.github/dcoir_review/scripts/dcoir_review/semantic_review_ledger_fingerprints.py'
        $ledgerHooksPath = '.github/dcoir_review/scripts/dcoir_review/semantic_review_ledger_hooks.py'
        foreach ($path in @($ledgerPath,$ledgerBuilderPath,$ledgerContractPath,$ledgerFingerprintsPath,$ledgerHooksPath)) {
            if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Stable semantic-review ledger file missing: $path" }
            $bytes=(Get-Item -LiteralPath $path).Length
            if ($bytes -gt 15000) { throw "Stable semantic-review ledger owner exceeds connector-safe size: $path=$bytes" }
        }
        $ledgerText = Get-Content -Raw -LiteralPath $ledgerPath
        $ledgerBuilderText = Get-Content -Raw -LiteralPath $ledgerBuilderPath
        $ledgerContractText = Get-Content -Raw -LiteralPath $ledgerContractPath
        $ledgerFingerprintsText = Get-Content -Raw -LiteralPath $ledgerFingerprintsPath
        $ledgerHooksText = Get-Content -Raw -LiteralPath $ledgerHooksPath
        if (($ledgerText + $ledgerBuilderText + $ledgerContractText + $ledgerFingerprintsText + $ledgerHooksText) -match 'dcoir_review_required_runtime_patch_v42') { throw 'Stable semantic-review ledger family still imports historical v42 ownership' }
        foreach ($literal in @('VERSION = "v42"','SEMANTIC_LEDGER_CONTRACT = "architecture-b-semantic-ledger-v1"','SEMANTIC_LEDGER_MARKER_PREFIX = "DCOIR semantic ledger: "','SEMANTIC_LEDGER_ATTR = "_dcoir_v42_semantic_review_ledger"')) { if ($ledgerContractText -notmatch [regex]::Escape($literal)) { throw "v42 ledger compatibility contract missing: $literal" } }
        foreach ($literal in @('dependency-context-v1','not-expanded-v42','semantic-result reuse is intentionally disabled in the v42 foundation','semantic-result reuse not enabled in v42')) { if ($ledgerBuilderText -notmatch [regex]::Escape($literal)) { throw "v42 builder compatibility data missing: $literal" } }
        foreach ($literal in @('_dcoir_review_semantic_ledger_original_hybrid_first_pass','_dcoir_review_semantic_ledger_original_append_context_to_review_body','_dcoir_review_semantic_ledger_original_write_debug_json_artifact_safely')) { if ($ledgerHooksText -notmatch [regex]::Escape($literal)) { throw "Stable ledger hook marker missing: $literal" } }
        if ($ledgerHooksText -match '_dcoir_v42_original_') { throw 'Stable ledger hooks retain private v42 implementation storage markers' }
        foreach ($literal in @('from dcoir_review.semantic_review_ledger_contract import','from dcoir_review.semantic_review_ledger_hooks import','from dcoir_review.semantic_review_ledger_builder import')) { if ($ledgerText -notmatch [regex]::Escape($literal)) { throw "Stable ledger owner dependency missing: $literal" } }
        foreach ($consumer in @('.github/dcoir_review/scripts/dcoir_review/semantic_result_reuse.py','.github/dcoir_review/scripts/dcoir_review/semantic_result_reuse_support.py','.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v44_telemetry.py','.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v46.py','.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v46_context.py')) { $consumerText=Get-Content -Raw -LiteralPath $consumer; if ($consumerText -match 'dcoir_review_required_runtime_patch_v42') { throw "Downstream consumer retains historical v42 dependency: $consumer" } }
        $frontierLedgerIndex = [Array]::IndexOf($sequence, 'dcoir_review.incremental_review_frontier')
        $ledgerIndex = [Array]::IndexOf($sequence, 'dcoir_review.semantic_review_ledger')
        $v43AfterLedgerIndex = [Array]::IndexOf($sequence, 'dcoir_review.semantic_result_reuse')
        if ($frontierLedgerIndex -lt 0 -or $ledgerIndex -ne ($frontierLedgerIndex + 1) -or $v43AfterLedgerIndex -ne ($ledgerIndex + 1)) { throw "Semantic-review ledger composition mismatch: frontier=$frontierLedgerIndex ledger=$ledgerIndex v43=$v43AfterLedgerIndex" }
        $ledgerBytes = (Get-Item -LiteralPath $ledgerPath).Length
        $ledgerBuilderBytes = (Get-Item -LiteralPath $ledgerBuilderPath).Length
        $ledgerContractBytes = (Get-Item -LiteralPath $ledgerContractPath).Length
        $ledgerFingerprintsBytes = (Get-Item -LiteralPath $ledgerFingerprintsPath).Length
        $ledgerHooksBytes = (Get-Item -LiteralPath $ledgerHooksPath).Length

        $historicalV43 = @(
            '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v43.py',
            '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v43_reuse.py',
        ...
        foreach ($path in $historicalV43) { if (Test-Path -LiteralPath $path) { throw "Retired historical v43 owner still exists: $path" } }
        $reuseOwnerPath = '.github/dcoir_review/scripts/dcoir_review/semantic_result_reuse.py'
        $reuseSupportPath = '.github/dcoir_review/scripts/dcoir_review/semantic_result_reuse_support.py'
        foreach ($path in @($reuseOwnerPath,$reuseSupportPath)) {
            if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Stable semantic-result reuse file missing: $path" }
            $bytes=(Get-Item -LiteralPath $path).Length
            if ($bytes -gt 15000) { throw "Stable semantic-result reuse owner exceeds connector-safe size: $path=$bytes" }
        }
        $reuseOwnerText = Get-Content -Raw -LiteralPath $reuseOwnerPath
        $reuseSupportText = Get-Content -Raw -LiteralPath $reuseSupportPath
        if (($reuseOwnerText + $reuseSupportText) -match 'dcoir_review_required_runtime_patch_v43') { throw 'Stable semantic-result reuse family still imports historical v43 ownership' }
        foreach ($literal in @('VERSION = "v43"','REUSE_CONTRACT = "architecture-b-semantic-result-reuse-v1"','DEPENDENCY_CONTRACT = "dependency-context-v2"','DEPENDENCY_MODE = "exact-semantic-prompt-v1"','MANIFEST_PATH = "metadata/semantic-result-reuse-manifest.json"')) { if (($reuseOwnerText + $reuseSupportText) -notmatch [regex]::Escape($literal)) { throw "v43 reuse compatibility contract missing: $literal" } }        foreach ($literal in @('_dcoir_review_semantic_result_reuse_state','_dcoir_review_semantic_result_reuse_applied')) { if ($reuseOwnerText -notmatch [regex]::Escape($literal)) { throw "Stable semantic-result reuse marker missing: $literal" } }
        if ($reuseOwnerText -match '_dcoir_v43_(?:reuse_state|applied)') { throw 'Stable semantic-result reuse owner retains private v43 implementation marker' }
        foreach ($consumer in @('.github/dcoir_review/scripts/dcoir_review/verified_finding_gate_prior.py','.github/dcoir_review/scripts/dcoir_review_architecture_b_benchmark.py','.github/dcoir_review/scripts/dcoir_review_architecture_b_benchmark_contract.py','.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v46_context.py','.github/dcoir_review/scripts/dcoir_review_review_context_ledger_integration_selftest.py','.github/dcoir_review/scripts/dcoir_review_verified_finding_gate_selftest.py')) { $consumerText=Get-Content -Raw -LiteralPath $consumer; if ($consumerText -match 'dcoir_review_required_runtime_patch_v43') { throw "Downstream consumer retains historical v43 dependency: $consumer" } }
        $ledgerBeforeReuseIndex = [Array]::IndexOf($sequence, 'dcoir_review.semantic_review_ledger')
        $reuseIndex = [Array]::IndexOf($sequence, 'dcoir_review.semantic_result_reuse')
        $v44AfterReuseIndex = [Array]::IndexOf($sequence, 'dcoir_review_required_runtime_patch_v44')
        if ($ledgerBeforeReuseIndex -lt 0 -or $reuseIndex -ne ($ledgerBeforeReuseIndex + 1) -or $v44AfterReuseIndex -ne ($reuseIndex + 1)) { throw "Semantic-result reuse composition mismatch: ledger=$ledgerBeforeReuseIndex reuse=$reuseIndex v44=$v44AfterReuseIndex" }
        $reuseOwnerBytes = (Get-Item -LiteralPath $reuseOwnerPath).Length
        $reuseSupportBytes = (Get-Item -LiteralPath $reuseSupportPath).Length
        $historicalV52 = @(
        ...
        @(
            ...
            'retired=v15,v19,v21,v22,v23,v24,v25,v26,v27,v28,v29,v34,v37,v38,v39,v41,v42,v43,v45,v47,v48,v50,v51,v52,v55,v58',
            'semantic_review_ledger_owner=dcoir_review.semantic_review_ledger',
            'semantic_review_ledger_helpers=builder,contract,fingerprints,hooks',
            'semantic_review_ledger_composition=incremental_review_frontier->semantic_review_ledger->semantic_result_reuse',
            'v42_compatibility_contract_attr_dependency_reuse=preserved',
            "semantic_review_ledger_bytes=$ledgerBytes",
            "semantic_review_ledger_builder_bytes=$ledgerBuilderBytes",
            "semantic_review_ledger_contract_bytes=$ledgerContractBytes",
            "semantic_review_ledger_fingerprints_bytes=$ledgerFingerprintsBytes",
            "semantic_review_ledger_hooks_bytes=$ledgerHooksBytes",
            'semantic_result_reuse_owner=dcoir_review.semantic_result_reuse',
            'semantic_result_reuse_helper=semantic_result_reuse_support',
            'semantic_result_reuse_composition=semantic_review_ledger->semantic_result_reuse->v44',
            'v43_compatibility_version_contract_dependency_manifest=preserved',
            "semantic_result_reuse_bytes=$reuseOwnerBytes",
            "semantic_result_reuse_support_bytes=$reuseSupportBytes",            'incremental_review_frontier_owner=dcoir_review.incremental_review_frontier',
            'incremental_review_frontier_helpers=hooks,state,scope',
            'incremental_review_frontier_composition=v31->incremental_review_frontier->semantic_review_ledger->semantic_result_reuse',
