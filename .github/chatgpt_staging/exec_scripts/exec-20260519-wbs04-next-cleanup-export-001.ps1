...
$requestId = 'exec-20260519-wbs04-next-cleanup-export-001'
$tableList = 'Plans,Idea Inbox,Validation Test Cases,GitHub Workflow Inventory,dcoir-memory-preflight,dcoir-decision-policy,dcoir-validation-orchestrator'
$outputPrefix = 'dcoir_wbs04_next_cleanup_export'
...
$summary = [ordered]@{
    schema = 'dcoir.wbs04.next_cleanup_export.readback.v1'
    request_id = $requestId
    success = $true
    purpose = 'Read-only WBS04 next cleanup workflow export for Plans, Idea Inbox, Validation Test Cases, GitHub Workflow Inventory, and confirmed helper-memory tables.'
    table_list = @('Plans','Idea Inbox','Validation Test Cases','GitHub Workflow Inventory','dcoir-memory-preflight','dcoir-decision-policy','dcoir-validation-orchestrator')
...
$summary | ConvertTo-Json -Depth 12 | Out-File -FilePath (Join-Path $readbackDir 'wbs04_next_cleanup_export_summary.json') -Encoding utf8

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add('# WBS04 next cleanup Airtable export readback')
$lines.Add('')
...
$lines.Add('- scope: Plans, Idea Inbox, Validation Test Cases, GitHub Workflow Inventory, and confirmed helper-memory tables')
...
$lines | Out-File -FilePath (Join-Path $readbackDir 'wbs04_next_cleanup_export_report.md') -Encoding utf8
