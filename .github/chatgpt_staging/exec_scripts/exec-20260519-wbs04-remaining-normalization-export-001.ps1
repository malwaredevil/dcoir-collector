...
$requestId = 'exec-20260519-wbs04-remaining-normalization-export-001'
$tableList = 'DCOIR Cleanup WBS,DCOIR Cleanup Scaffold Registry,Gemini Research Reference,Admin Registry,Work Items,Session Checkpoints,Validation Evidence,Operator Tools Registry,Local Configuration Registry'
$outputPrefix = 'dcoir_wbs04_remaining_normalization_export'
...
$summary = [ordered]@{
    schema = 'dcoir.wbs04.remaining_normalization_export.readback.v1'
    request_id = $requestId
    success = $true
    purpose = 'Read-only WBS04 remaining retention/status normalization workflow export. This is metadata-normalization intake only; merge/delete-candidate pass remains separate.'
    table_list = @('DCOIR Cleanup WBS','DCOIR Cleanup Scaffold Registry','Gemini Research Reference','Admin Registry','Work Items','Session Checkpoints','Validation Evidence','Operator Tools Registry','Local Configuration Registry')
...
$summary | ConvertTo-Json -Depth 12 | Out-File -FilePath (Join-Path $readbackDir 'wbs04_remaining_normalization_export_summary.json') -Encoding utf8

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add('# WBS04 remaining retention/status normalization export readback')
$lines.Add('')
...
$lines.Add('- scope: remaining metadata-normalization surfaces before separate merge/delete-candidate pass')
...
$lines | Out-File -FilePath (Join-Path $readbackDir 'wbs04_remaining_normalization_export_report.md') -Encoding utf8
