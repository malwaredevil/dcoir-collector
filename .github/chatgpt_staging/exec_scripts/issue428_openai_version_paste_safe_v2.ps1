...
try {
    ...
    $tempScript = Join-Path $env:TEMP 'issue428_openai_version_paste_safe_v2_inner.ps1'
    $rewrite = Join-Path $env:TEMP 'issue428_rewrite_script.py'
@'
...
'@ | Set-Content -LiteralPath $rewrite -Encoding UTF8
    ...
    python $rewrite
    if ($LASTEXITCODE -ne 0) { throw "Failed to prepare marker-safe issue #428 retry script." }
