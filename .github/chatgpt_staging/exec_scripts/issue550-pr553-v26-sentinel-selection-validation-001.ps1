...
$expectedHead = 'bcabb8d69f9c2a52399c533fce812fdaf3030f8d'
$base = '9dc055d81fba2bcd4e840ad1e9c9645bdc6a6a20'
$branch = 'refactor/issue-550-dcoir-runtime-consolidation'
$requestRel = '.github/chatgpt_staging/exec_requests/issue550-pr553-v26-sentinel-selection-validation-001.json'
...
$worktree = Join-Path $runnerTemp ('issue550-pr553-v26-validation-' + [Guid]::NewGuid().ToString('N'))
$created = $false
try {
    ...
    try {
        ...
        foreach ($retired in @('dcoir_review_required_runtime_patch_v25','dcoir_review_required_runtime_patch_v26','dcoir_review_required_runtime_patch_v27','dcoir_review_required_runtime_patch_v28','dcoir_review_required_runtime_patch_v29')) {
        ...
        if ($repairIndex -lt 0 -or $sentinelIndex -ne ($repairIndex + 1) -or $v30Index -ne ($sentinelIndex + 1)) {
            throw "Stable sentinel selection composition order mismatch: repair=$repairIndex sentinel=$sentinelIndex v30=$v30Index"
        }
        ...
        foreach ($stableOwner in @('dcoir_review.repair_pipeline','dcoir_review.repair_reliability','dcoir_review.repair_render','dcoir_review.sentinel_selection')) {
        ...
        $summaryPath = Join-Path $downloads 'issue550-pr553-v26-sentinel-selection-validation-001-summary.txt'
        @(
            'result=PASS',
...
Write-Output 'ISSUE550_PR553_V26_SENTINEL_SELECTION_VALIDATION_001_PASS'
