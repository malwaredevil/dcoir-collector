...
$summaryPath = '.github/chatgpt_staging/status_reports/chatgpt-exec/exec-20260626-dcoir-review-hybrid-main-007/diagnostic-summary.md'
...
$innerText = $innerText.Replace("git diff --check`nif (`$LASTEXITCODE -ne 0) { throw 'git diff --check failed' }", "git -c core.whitespace=blank-at-eol,blank-at-eof,space-before-tab,cr-at-eol diff --check -- .github/dcoir_review/scripts/openrouter_pr_review_pareto_context.py .github/dcoir_review/scripts/openrouter_pr_review_pareto_context_selftest.py .github/dcoir_review/openrouter-pr-review-pareto.yml .github/ops/requests/apply_patch/README.md`nif (`$LASTEXITCODE -ne 0) { throw 'git diff --check failed' }")
