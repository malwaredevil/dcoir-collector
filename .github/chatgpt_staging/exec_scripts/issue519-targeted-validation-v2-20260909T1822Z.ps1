...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-issue519-targeted-v2-' + $requestedSha.Substring(0,12))
...
try {
    ...
    } | ConvertTo-Json -Depth 4 | Out-File -LiteralPath (Join-Path $downloads 'issue519-targeted-validation-v2-receipt.json') -Encoding utf8

    Write-Host 'Issue #519 targeted deterministic validation v2 passed.'
}
finally {
