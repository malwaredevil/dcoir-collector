...
$requestedSha = 'fff03172a93ff1d332465df476d2a441d6eb771d'
...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-pr493-' + $requestedSha.Substring(0,12))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'PR #493 credit-aware source py_compile failed' }
    ...
    @{
        ...
        Out-File -LiteralPath (Join-Path $downloads 'issue457-pr493-exact-head-validation-receipt.json') -Encoding utf8

    Write-Host 'Issue #457 / PR #493 exact-head deterministic validation passed.'
}
finally {
