...
$requestedSha = '2a2b38b206c9b8cdd5a8df61d5c72ae268aaa8cc'
$requestedBranch = 'issue-515-bounded-structured-recovery'
$expectedValidationCount = 51
$expectedChangedPythonCount = 7
...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-pr516-v1-' + $requestedSha.Substring(0,12))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'PR #516 changed Python source/selftest py_compile failed' }
    ...
    @{
        schema = 'dcoir.issue515.pr516_exact_head_validation.v1'
        ...
        Out-File -LiteralPath (Join-Path $downloads 'issue515-pr516-exact-head-validation-v1-receipt.json') -Encoding utf8

    Write-Host 'Issue #515 / PR #516 exact-head deterministic validation v1 passed.'
}
finally {
