...
$expectedHead = '11a7297678ab65930da4ff96bb64af6bed51388b'
$base = '9dc055d81fba2bcd4e840ad1e9c9645bdc6a6a20'
$branch = 'refactor/issue-550-dcoir-runtime-consolidation'
$requestRel = '.github/chatgpt_staging/exec_requests/issue550-pr553-v24-verified-finding-render-validation-001.json'
...
$worktree = Join-Path $runnerTemp ('issue550-pr553-v24-validation-' + [Guid]::NewGuid().ToString('N'))
$created = $false
try {
    ...
    try {
        ...
        foreach ($retired in @('dcoir_review_required_runtime_patch_v24','dcoir_review_required_runtime_patch_v25','dcoir_review_required_runtime_patch_v26','dcoir_review_required_runtime_patch_v27','dcoir_review_required_runtime_patch_v28','dcoir_review_required_runtime_patch_v29')) {
            if ($sequence -contains $retired) { throw "Retired module still in production sequence: $retired" }
        }
        if ($sequence -notcontains 'dcoir_review.verified_finding_render') { throw 'Stable verified finding renderer missing from production sequence' }
        if ($sequence -notcontains 'dcoir_review.repair_pipeline') { throw 'Canonical repair pipeline missing from production sequence' }
        if ($sequence -notcontains 'dcoir_review.sentinel_selection') { throw 'Stable sentinel selection owner missing from production sequence' }
        $v23Index = [Array]::IndexOf($sequence, 'dcoir_review_required_runtime_patch_v23')
        $verifiedRenderIndex = [Array]::IndexOf($sequence, 'dcoir_review.verified_finding_render')
        ...
        if ($v23Index -lt 0 -or $verifiedRenderIndex -ne ($v23Index + 1) -or $repairIndex -ne ($verifiedRenderIndex + 1) -or $sentinelIndex -ne ($repairIndex + 1) -or $v30Index -ne ($sentinelIndex + 1)) {
            throw "Stable v24/v26 composition order mismatch: v23=$v23Index verified_render=$verifiedRenderIndex repair=$repairIndex sentinel=$sentinelIndex v30=$v30Index"
        }
        if ($sequence -contains 'dcoir_review_required_runtime_patch_v24') { throw 'Retired v24 module still in production sequence' }
        if ($sequence -contains 'dcoir_review_required_runtime_patch_v26') { throw 'Retired v26 module still in production sequence' }
        if ($sequence -contains 'dcoir_review_required_runtime_patch_v28') { throw 'Retired v28 module still in production sequence' }
        foreach ($stableOwner in @('dcoir_review.verified_finding_render','dcoir_review.repair_pipeline','dcoir_review.repair_reliability','dcoir_review.repair_render','dcoir_review.sentinel_selection')) {
        ...
        $summaryPath = Join-Path $downloads 'issue550-pr553-v24-verified-finding-render-validation-001-summary.txt'
        @(
            'result=PASS',
            ...
            'retired=v24,v25,v26,v27,v28,v29',
...
Write-Output 'ISSUE550_PR553_V24_VERIFIED_FINDING_RENDER_VALIDATION_001_PASS'
