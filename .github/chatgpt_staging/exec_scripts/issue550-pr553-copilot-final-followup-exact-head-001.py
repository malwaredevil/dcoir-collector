...
EXPECTED_HEAD = "70a07e104e8b70b9e9410833d857515a05dc957b"
EXPECTED_PATCH_SHA = "003f70609322940eb6b9031dda55af54dc7418b502162b40a859039e11bdba44"
REQUEST_ID = "issue550-pr553-copilot-final-followup-exact-head-001"
TERMINAL = "ISSUE550_PR553_COPILOT_FINAL_FOLLOWUP_EXACT_HEAD_001_PASS"
...
run("git", "cat-file", "-e", f"{EXPECTED_HEAD}^{{commit}}", cwd=repo)
...
try:
    actual_head = out("git", "rev-parse", "HEAD", cwd=worktree)
    actual_parent = out("git", "rev-parse", "HEAD^", cwd=worktree)
    if actual_head != EXPECTED_HEAD: raise RuntimeError(f"detached head drift: {actual_head}")
    if actual_parent != SOURCE_PARENT: raise RuntimeError(f"parent drift: {actual_parent}")
    committed = subprocess.check_output(["git", "diff", "--binary", SOURCE_PARENT, EXPECTED_HEAD, "--"], cwd=worktree)
    committed_sha = hashlib.sha256(committed).hexdigest()
    if len(committed) != 8154 or committed_sha != EXPECTED_PATCH_SHA:
        raise RuntimeError(f"committed frozen diff drift: bytes={len(committed)} sha={committed_sha}")
    ...
    final_bytes = subprocess.check_output(["git", "diff", "--binary", SOURCE_PARENT, EXPECTED_HEAD, "--"], cwd=worktree)
    final_sha = hashlib.sha256(final_bytes).hexdigest()
    if len(final_bytes) != 8154: raise RuntimeError(f"post-validation committed byte drift: {len(final_bytes)}")
