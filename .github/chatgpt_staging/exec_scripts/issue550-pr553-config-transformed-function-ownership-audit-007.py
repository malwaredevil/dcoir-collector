...
new_dirty = "    $dirty = @(& git status --porcelain)\n    if ($dirty.Count -eq 0) { throw 'Transformed audit worktree unexpectedly clean' }\n    $changed = @(& git diff --name-only HEAD)\n    if ($changed -notcontains '.github/dcoir_review/scripts/dcoir_review/review_config.py') { throw 'Transformed audit did not include canonical review_config.py' }"
...
text = text.replace('ISSUE550_PR553_FUNCTION_OWNERSHIP_AUDIT_001_PASS', 'ISSUE550_PR553_CONFIG_TRANSFORMED_FUNCTION_OWNERSHIP_AUDIT_007_PASS', 1)
