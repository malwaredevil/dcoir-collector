...
$expectedHead = 'efabcaec43676951a596afed06c601dc8486d840'
$base = '9dc055d81fba2bcd4e840ad1e9c9645bdc6a6a20'
$branch = 'refactor/issue-550-dcoir-runtime-consolidation'
$requestRel = '.github/chatgpt_staging/exec_requests/issue550-pr553-status-cutover-validation-001.json'
...
$worktree = Join-Path $runnerTemp (
    'dcoir-pr553-status-validation-' + [Guid]::NewGuid().ToString('N')
)
...
try {
    ...
    try {
        ...
        $compileCommand = 'python -m py_compile ' + ($compileFiles -join ' ')
        ...
        $inventoryPath = Join-Path $downloads 'dcoir-review-status-cutover-inventory.json'
        $inventoryOutput = @(
        ...
        if ([int]$inventory.production_patch_count -ne 59) {
            throw "Production patch sequence changed unexpectedly: $($inventory.production_patch_count)"
        }

        $provenancePath = Join-Path $downloads 'dcoir-review-status-cutover-provenance.json'
        $provenanceOutput = @(
        ...
        if (@($provenance.production_patch_sequence).Count -ne 59) {
            throw "Runtime provenance observed an unexpected production patch count: $(@($provenance.production_patch_sequence).Count)"
        }
...
Write-Host "PR #553 status-cutover validation passed at exact head: $expectedHead"
