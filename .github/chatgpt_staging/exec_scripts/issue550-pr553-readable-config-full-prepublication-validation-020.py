...
inner = downloads / "issue550-pr553-readable-config-full-prepublication-validation-020-inner.py"
...
replacements = {
    ...
    'print("ISSUE550_PR553_READABLE_CONFIG_FULL_PREPUBLICATION_VALIDATION_012_PASS")':
        'print("ISSUE550_PR553_READABLE_CONFIG_FULL_PREPUBLICATION_VALIDATION_020_PASS")',
}
...
size_old = '''    review_config = scripts / "dcoir_review" / "review_config.py"\n    if review_config.stat().st_size != 9251:\n        raise RuntimeError(f"readable review_config byte-size drift: {review_config.stat().st_size}")\n'''
size_new = '''    review_config = scripts / "dcoir_review" / "review_config.py"\n    if review_config.stat().st_size > 12000:\n        raise RuntimeError(f"readable review_config exceeds 12 KB maintainability bound: {review_config.stat().st_size}")\n'''
if source.count(size_old) != 1:
    raise SystemExit(f"review_config size-bound replacement anchor count={source.count(size_old)}")
source = source.replace(size_old, size_new, 1)
...
print("FULL_PREPUBLICATION_VALIDATION_020_WRAPPER_PASS")
