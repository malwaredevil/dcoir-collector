...
$requestId = 'exec-20260520-wbs04-merge-delete-batch2-export-001'
$tableList = 'Gemini Research Reference,Governance Control Plane,Repo Surface Registry,dcoir-memory-preflight,dcoir-decision-policy,dcoir-validation-orchestrator,Validation Evidence'
$outputPrefix = 'dcoir_wbs04_merge_delete_batch2_export'
...
$summary = [ordered]@{
    ...
    success = $true
    purpose = 'Read-only WBS04 merge/delete candidate analysis export, batch 2 restart. Full rows and fields for aggressive row reduction analysis. Delete Queue and Batch 1 tables intentionally excluded.'
    table_batch = 2
    table_batch_total = 3
    table_list = @('Gemini Research Reference','Governance Control Plane','Repo Surface Registry','dcoir-memory-preflight','dcoir-decision-policy','dcoir-validation-orchestrator','Validation Evidence')
    excluded_tables = @('Delete Queue','Work Items','Session Checkpoints','Idea Inbox','Plans','Operator Preferences','Validation Test Cases','Queue Control')
...
$summary | ConvertTo-Json -Depth 12 | Out-File -FilePath (Join-Path $readbackDir 'wbs04_merge_delete_batch2_export_summary.json') -Encoding utf8

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add('# WBS04 merge/delete candidate export readback - batch 2 restart')
$lines.Add('')
...
$lines.Add('- scope: batch 2 restart; full-row/full-field intake for aggressive merge/update/delete-candidate analysis')
$lines.Add('- excluded: Delete Queue plus Batch 1 tables')
...
$lines | Out-File -FilePath (Join-Path $readbackDir 'wbs04_merge_delete_batch2_export_report.md') -Encoding utf8
