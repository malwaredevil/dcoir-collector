...
$requestId = 'exec-20260519-wbs04-post-first-four-export-002'
$tableList = 'DCOIR Lifecycle Ledger,Queue Control,Governance Control Plane,Operator Preferences,Repo Surface Registry,Local Configuration Registry,Operator Tools Registry'
$outputPrefix = 'dcoir_wbs04_post_first_four_export'
...
$summary = [ordered]@{
    schema = 'dcoir.wbs04.post_first_four_export.readback.v1'
    request_id = $requestId
    success = $true
    purpose = 'Read-only WBS04 post-first-four workflow export. Delete Queue is intentionally excluded because approved queue rows auto-delete after processing.'
    table_list = @('DCOIR Lifecycle Ledger','Queue Control','Governance Control Plane','Operator Preferences','Repo Surface Registry','Local Configuration Registry','Operator Tools Registry')
...
$summary | ConvertTo-Json -Depth 12 | Out-File -FilePath (Join-Path $readbackDir 'wbs04_post_first_four_export_summary.json') -Encoding utf8

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add('# WBS04 post-first-four Airtable export readback')
$lines.Add('')
...
$lines.Add('- excluded: Delete Queue, because approved queue rows auto-delete after processing')
...
$lines | Out-File -FilePath (Join-Path $readbackDir 'wbs04_post_first_four_export_report.md') -Encoding utf8
