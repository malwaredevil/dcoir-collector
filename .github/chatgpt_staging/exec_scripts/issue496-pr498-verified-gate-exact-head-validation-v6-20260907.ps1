...
$requestedSha = '8eb03bb2b035cc2f48b550a63961fb22c581d9d7'
...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-pr498-v6-' + $requestedSha.Substring(0,12))
...
try {
    ...
    if ($pythonFiles.Count -ne 9) { throw "expected 9 changed Python files, found $($pythonFiles.Count)" }
    ...
    @{
        ...
        Out-File -LiteralPath (Join-Path $downloads 'issue496-pr498-exact-head-validation-v6-receipt.json') -Encoding utf8

    Write-Host 'Issue #496 / PR #498 exact-head deterministic validation v6 passed.'
}
finally {
