...
x=x.replace(old,new,1)
old="""        fallback_config = fake.load_pareto_context_config("unused")
        assert fallback_config.model == "model-a"
        assert v54._telemetry_error_count(fallback_config) >= 1
"""
new="""        # Loader-side initialization now belongs to canonical review_config, not v54.
        ...
        assert v54._telemetry_error_count(fallback_config) >= 1
"""
if old not in x: raise RuntimeError('v54 canonical loader fail-soft anchor missing')
x=x.replace(old,new,1)
old="""    assert not hasattr(broken_module, v54.LOAD_STORAGE)
"""
if old not in x: raise RuntimeError('v54 retired loader-storage assertion missing')
x=x.replace(old,"",1)
wr(v54test,x)
'''
