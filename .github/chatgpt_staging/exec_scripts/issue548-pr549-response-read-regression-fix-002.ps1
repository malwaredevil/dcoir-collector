...
$requestId = 'issue548-pr549-response-read-regression-fix-002'
...
try {
    ...
    try {
        ...
        $patcher = Join-Path $env:RUNNER_TEMP "$requestId-patch.py"
        $patchScript = @'
...
def replace_once(source: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise SystemExit(f"{label} expected exactly once; observed {count}")
...
''',
    "production failure comment replacement",
)
...
text = replace_once(
    ...
    "primary response.read failure replacement",
)
...
text = replace_once(
    ...
    "direct response.read failure replacement",
)
...
text = replace_once(
    ...
    "production wrapper response.read failure replacement",
)

path.write_text(text, encoding="utf-8", newline="\n")
'@
        [IO.File]::WriteAllText($patcher,$patchScript,(New-Object Text.UTF8Encoding($false)))
        Invoke-CheckedNative 'Apply response.read regression correction' { & $pythonExe $patcher $targetPath }
