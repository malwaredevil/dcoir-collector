...
$text = $text.Replace('07cf4c1e11f833c7273d059c9a0fc367e9b4de53', 'f7ca5be780d52682a9e9a6740e302c19b52ba8b5')
$text = $text.Replace('issue550-pr553-repair-consolidation-validation-001.json', 'issue550-pr553-repair-consolidation-validation-004.json')
$text = $text.Replace('issue550-pr553-repair-consolidation-validation-001-summary.txt', 'issue550-pr553-repair-consolidation-validation-004-summary.txt')
$text = $text.Replace('ISSUE550_PR553_REPAIR_CONSOLIDATION_VALIDATION_PASS', 'ISSUE550_PR553_REPAIR_CONSOLIDATION_VALIDATION_004_PASS')

$tempRoot = if ([string]::IsNullOrWhiteSpace($env:RUNNER_TEMP)) { [IO.Path]::GetTempPath() } else { $env:RUNNER_TEMP }
$tempScript = Join-Path $tempRoot ('issue550-pr553-validation-004-' + [Guid]::NewGuid().ToString('N') + '.ps1')
try {
