...
$requestedSha = '0079fc141214ae8d1cebea46ce7799147f422839'
$requestedBranch = 'issue-497-preserve-candidate-identity'
$expectedValidationCount = 50
...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-pr499-v1-' + $requestedSha.Substring(0,12))
...
try {
    ...
    $pythonFiles = @(
        ...
        '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v51.py',
        '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v51_selftest.py'
    )
    if ($pythonFiles.Count -ne 3) { throw "expected 3 changed Python files, found $($pythonFiles.Count)" }
    & python3 -m py_compile @pythonFiles
    if ($LASTEXITCODE -ne 0) { throw 'PR #499 changed Python source/selftest py_compile failed' }
    ...
    @{
        schema = 'dcoir.issue497.pr499_exact_head_validation.v1'
        ...
        Out-File -LiteralPath (Join-Path $downloads 'issue497-pr499-exact-head-validation-v1-receipt.json') -Encoding utf8

    Write-Host 'Issue #497 / PR #499 exact-head deterministic validation v1 passed.'
}
finally {
