...
$expectedHead = '2e6d47a3d25e5ccb33d0a91fbc1bb260ff266c94'
$base = '9dc055d81fba2bcd4e840ad1e9c9645bdc6a6a20'
$branch = 'refactor/issue-550-dcoir-runtime-consolidation'
$requestRel = '.github/chatgpt_staging/exec_requests/issue550-pr553-v22-quality-gate-validation-001.json'
...
$worktree = Join-Path $runnerTemp ('issue550-pr553-v22-validation-' + [Guid]::NewGuid().ToString('N'))
$created = $false
try {
    ...
    try {
        ...
        foreach ($retired in @('dcoir_review_required_runtime_patch_v15','dcoir_review_required_runtime_patch_v22','dcoir_review_required_runtime_patch_v23','dcoir_review_required_runtime_patch_v24','dcoir_review_required_runtime_patch_v25','dcoir_review_required_runtime_patch_v26','dcoir_review_required_runtime_patch_v27','dcoir_review_required_runtime_patch_v28','dcoir_review_required_runtime_patch_v29')) {
            if ($sequence -contains $retired) { throw "Retired module still in production sequence: $retired" }
        }
        foreach ($stableOwner in @('dcoir_review.finding_family','dcoir_review.quality_gate','dcoir_review.normalized_finding_selection','dcoir_review.verified_finding_render','dcoir_review.repair_pipeline','dcoir_review.sentinel_selection')) {
        ...
        $v21Index = [Array]::IndexOf($sequence, 'dcoir_review_required_runtime_patch_v21')
        $qualityGateIndex = [Array]::IndexOf($sequence, 'dcoir_review.quality_gate')
        ...
        if ($v21Index -lt 0 -or $qualityGateIndex -ne ($v21Index + 1) -or $normalizedIndex -ne ($qualityGateIndex + 1) -or $verifiedIndex -ne ($normalizedIndex + 1) -or $repairIndex -ne ($verifiedIndex + 1) -or $sentinelIndex -ne ($repairIndex + 1) -or $v30Index -ne ($sentinelIndex + 1)) {
            throw "Stable quality/selection/render/repair composition order mismatch: v21=$v21Index quality_gate=$qualityGateIndex normalized=$normalizedIndex verified=$verifiedIndex repair=$repairIndex sentinel=$sentinelIndex v30=$v30Index"
        }
        foreach ($stableOwner in @('dcoir_review.finding_family','dcoir_review.quality_gate','dcoir_review.normalized_finding_selection','dcoir_review.verified_finding_render','dcoir_review.repair_pipeline','dcoir_review.repair_reliability','dcoir_review.repair_render','dcoir_review.sentinel_selection')) {
        ...
        foreach ($retiredPath in @(
            ...
            '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v22.py',
        ...
        $summaryPath = Join-Path $downloads 'issue550-pr553-v22-quality-gate-validation-001-summary.txt'
        @(
            'result=PASS',
            ...
            'retired=v15,v22,v23,v24,v25,v26,v27,v28,v29',
            'quality_gate_owner=dcoir_review.quality_gate',
...
Write-Output 'ISSUE550_PR553_V22_QUALITY_GATE_VALIDATION_001_PASS'
