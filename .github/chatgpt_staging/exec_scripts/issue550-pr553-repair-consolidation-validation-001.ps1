...
$expectedHead = '07cf4c1e11f833c7273d059c9a0fc367e9b4de53'
$base = '9dc055d81fba2bcd4e840ad1e9c9645bdc6a6a20'
$branch = 'refactor/issue-550-dcoir-runtime-consolidation'
$requestRel = '.github/chatgpt_staging/exec_requests/issue550-pr553-repair-consolidation-validation-001.json'
...
$requestHashBefore = (Get-FileHash -Algorithm SHA256 -LiteralPath $requestPath).Hash
...
try {
    ...
    try {
        ...
        if ([int]$inventory.production_patch_count -ne 57) { throw "Expected 57 production components, observed $($inventory.production_patch_count)" }
        ...
        foreach ($retired in @('dcoir_review_required_runtime_patch_v25','dcoir_review_required_runtime_patch_v27','dcoir_review_required_runtime_patch_v29')) {
        ...
        if (@($provenance.production_patch_sequence).Count -ne 57) { throw 'Runtime provenance production count mismatch' }
        foreach ($retired in @('dcoir_review_required_runtime_patch_v25','dcoir_review_required_runtime_patch_v27','dcoir_review_required_runtime_patch_v29')) {
        ...
        $summaryPath = Join-Path $downloads 'issue550-pr553-repair-consolidation-validation-001-summary.txt'
        @(
            'result=PASS',
...
$requestHashAfter = (Get-FileHash -Algorithm SHA256 -LiteralPath $requestPath).Hash
...
Write-Output 'ISSUE550_PR553_REPAIR_CONSOLIDATION_VALIDATION_PASS'
