...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-pr520-v3-' + $requestedSha.Substring(0,12))
...
try {
    ...
    @{
        schema = 'dcoir.issue519.pr520_exact_head_validation.v3'
        issue_number = 519
        pr_number = 520
    ...
    } | ConvertTo-Json -Depth 4 | Out-File -LiteralPath (Join-Path $downloads 'issue519-pr520-exact-head-validation-v3-receipt.json') -Encoding utf8

    Write-Host 'Issue #519 / PR #520 exact-head deterministic validation v3 passed.'
}
finally {
