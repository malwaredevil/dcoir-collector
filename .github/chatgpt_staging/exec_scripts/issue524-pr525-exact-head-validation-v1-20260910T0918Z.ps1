...
$requestedSha = 'bfda66fc572d979def91c4565e757983b211126a'
$requestedBranch = 'issue-524-adjudicator-shape-recovery'
$expectedValidationCount = 54
$expectedChangedPythonCount = 3
...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-pr525-v1-' + $requestedSha.Substring(0,12))
...
try {
    ...
    $pythonFiles = @(
        ...
        '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v55.py',
        '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v55_selftest.py'
    )
    if ($pythonFiles.Count -ne $expectedChangedPythonCount) {
        throw "expected $expectedChangedPythonCount changed Python files, found $($pythonFiles.Count)"
    }
    & python3 -m py_compile @pythonFiles
    if ($LASTEXITCODE -ne 0) { throw 'PR #525 changed Python source/selftest py_compile failed' }
    ...
    @{
        schema = 'dcoir.issue524.pr525_exact_head_validation.v1'
        validation_attempt = 'v1'
        issue_number = 524
        pr_number = 525
        ...
        Out-File -LiteralPath (Join-Path $downloads 'issue524-pr525-exact-head-validation-v1-receipt.json') -Encoding utf8

    Write-Host 'Issue #524 / PR #525 exact-head deterministic validation v1 passed.'
}
finally {
