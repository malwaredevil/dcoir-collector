...
EXPECTED_HEAD = "a9f77a3a5aaf9d1a12424919d8c5040d47d02b07"
EXPECTED_TREE = "c8ca24a56d4b6ca2ebc96bf3d55f11797009a7e3"
BRANCH = "refactor/issue-550-dcoir-runtime-consolidation"
REQUEST_ID = "issue550-pr553-canonical-finding-validation-prepublication-001"
TERMINAL = "ISSUE550_PR553_CANONICAL_FINDING_VALIDATION_PREPUBLICATION_001_PASS"
PATCH_PAYLOAD = ".github/chatgpt_staging/exec_payloads/issue550-pr553-canonical-finding-validation-prepublication-001.patch.gz.b64"
...
compressed_text = (repo / PATCH_PAYLOAD).read_text(encoding="utf-8")
...
try:
    ...
    run("git", "add", "-N", new_segment, cwd=worktree)
    ...
    patch_text = run("git", "diff", "--binary", cwd=worktree).stdout
    final_patch.write_text(patch_text, encoding="utf-8", newline="\n")
    final_sha = hashlib.sha256(final_patch.read_bytes()).hexdigest()
