...
try {
    ...
    Write-Host "Reviewed base ancestry verified: $reviewedBase"
    ...
    try {
        ...
        Invoke-CheckedNative -Label 'git diff --check (reviewed PR range)' -Action {
            git diff --check "$reviewedBase...$targetHead"
        }
