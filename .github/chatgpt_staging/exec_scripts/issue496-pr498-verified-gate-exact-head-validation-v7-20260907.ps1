...
$requestedSha = 'f2756820a14f759f82df007cd77ea7099e00f9c6'
...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-pr498-v7-' + $requestedSha.Substring(0,12))
...
try {
    ...
    if ($pythonFiles.Count -ne 11) { throw "expected 11 changed Python files, found $($pythonFiles.Count)" }
    ...
    @{
        ...
        Out-File -LiteralPath (Join-Path $downloads 'issue496-pr498-exact-head-validation-v7-receipt.json') -Encoding utf8

    Write-Host 'Issue #496 / PR #498 exact-head deterministic validation v7 passed.'
}
finally {
