...
$requestedSha = '8ec849472f15c98f07ef657a66a1f97d395b96b8'
...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-pr520-v7-' + $requestedSha.Substring(0,12))
try {
    ...
    @{schema='dcoir.issue519.pr520_exact_head_validation.v7';issue_number=519;pr_number=520;requested_branch=$requestedBranch;requested_head_sha=$requestedSha;remote_branch_head_sha=$remoteSha;actual_head_sha=$actualSha;source_pycompile='pass';changed_python_files_compiled=$pythonFiles.Count;governed_validation_commands=$commands.Count;governed_validation_result='pass';worktree_clean=$true;inference_credentials_removed=$true;live_model_inference=$false;live_dcoir_review_invocation=$false;github_review_publication=$false;pr_source_mutation=$false;ready_transition=$false;merge_performed=$false;source_content_matches_validated_parent='a10644242341bad0545a4d77aadb13e19b3a6b0c'} | ConvertTo-Json -Depth 4 | Out-File -LiteralPath (Join-Path $downloads 'issue519-pr520-exact-head-validation-v7-receipt.json') -Encoding utf8
    Write-Host 'Issue #519 / PR #520 exact-head deterministic validation v7 passed.'
