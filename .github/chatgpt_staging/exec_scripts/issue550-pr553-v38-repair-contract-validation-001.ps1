...
$expectedHead = '9a3ccdb5a05caa0bf3a7fc912a823a818c463e4f'
$expectedTree = 'aaa34fa72b5ede60dac62e4ee4e2f01a291b6ee6'
$base = '9dc055d81fba2bcd4e840ad1e9c9645bdc6a6a20'
$branch = 'refactor/issue-550-dcoir-runtime-consolidation'
$requestId = 'issue550-pr553-v38-repair-contract-validation-001'
$requestRel = ".github/chatgpt_staging/exec_requests/$requestId.json"
$requestPath = Join-Path $repo $requestRel
$terminalMarker = 'ISSUE550_PR553_V38_REPAIR_CONTRACT_VALIDATION_001_PASS'
...
$worktree = Join-Path $runnerTemp ('issue550-pr553-v38-repair-contract-validation-' + [Guid]::NewGuid().ToString('N'))
$created = $false
try {
    ...
    try {
        ...
        $historicalV38Path = '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v38.py'
        $historicalV38Selftest = '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v38_selftest.py'
        $historicalV38CriticSelftest = '.github/dcoir_review/scripts/dcoir_review_required_runtime_patch_v38_critic_selftest.py'
        if (Test-Path -LiteralPath $historicalV38Path) { throw 'Retired historical v38 production file still exists' }
        if (Test-Path -LiteralPath $historicalV38Selftest) { throw 'Retired historical v38 selftest still exists' }
        if (Test-Path -LiteralPath $historicalV38CriticSelftest) { throw 'Retired historical v38 critic selftest still exists' }

        $repairPath = '.github/dcoir_review/scripts/dcoir_review/repair_contract.py'
        $repairSelftest = '.github/dcoir_review/scripts/dcoir_review_repair_contract_selftest.py'
        $repairCriticSelftest = '.github/dcoir_review/scripts/dcoir_review_repair_contract_critic_selftest.py'
        if (-not (Test-Path -LiteralPath $repairPath -PathType Leaf)) { throw 'Stable repair_contract.py missing' }
        if (-not (Test-Path -LiteralPath $repairSelftest -PathType Leaf)) { throw 'Stable repair contract selftest missing' }
        if (-not (Test-Path -LiteralPath $repairCriticSelftest -PathType Leaf)) { throw 'Stable repair contract critic selftest missing' }
        $repairText = Get-Content -Raw -LiteralPath $repairPath
        if ($repairText -match '(?m)^VERSION\s*=\s*["'']v38["'']') { throw 'Stable repair contract still declares historical VERSION v38' }
        if ($repairText -notmatch 'APPLIED_MARKER\s*=\s*["'']_dcoir_review_repair_contract_applied["'']') { throw 'Stable repair contract apply marker missing' }
        if ($repairText -notmatch 'PROMPT_STORAGE\s*=\s*["'']_dcoir_review_repair_contract_original_repair_author_prompt["'']') { throw 'Stable repair contract prompt storage marker missing' }
        if ($repairText -notmatch 'CRITIC_MIN_CONFIDENCE\s*=\s*0\.95') { throw 'Stable repair critic confidence floor changed' }
        if ($repairText -notmatch 'AUTHOR_MIN_CONFIDENCE\s*=\s*0\.0') { throw 'Stable repair author advisory confidence changed' }
        $repairBytes = (Get-Item -LiteralPath $repairPath).Length
        if ($repairBytes -gt 15000) { throw "Stable repair contract exceeds connector-safe size: $repairBytes" }

        $entrypointText = Get-Content -Raw -LiteralPath '.github/dcoir_review/scripts/dcoir_review/entrypoint.py'
        if ($entrypointText -notmatch '["'']dcoir_review\.repair_contract["'']') { throw 'Entrypoint does not compose stable repair contract owner' }
        if ($entrypointText -match '["'']dcoir_review_required_runtime_patch_v38["'']') { throw 'Entrypoint still composes historical v38 owner' }
        ...
        if ($provSequence -contains 'dcoir_review_required_runtime_patch_v38') { throw 'Historical v38 appears in runtime provenance' }
        if ($provSequence -notcontains 'dcoir_review.repair_contract') { throw 'Stable repair contract owner missing from runtime provenance' }
        ...
        @(
            ...
            'repair_contract_owner=dcoir_review.repair_contract',
            ...
            "repair_contract_bytes=$repairBytes",
