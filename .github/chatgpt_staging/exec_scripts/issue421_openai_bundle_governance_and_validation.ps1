...
$worktree = Join-Path $env:RUNNER_TEMP 'issue421-openai-bundle-worktree'
...
if ($remoteHead -ne $expectedHead) {
  throw "Issue #421 branch moved before governed update: expected $expectedHead, found $remoteHead"
}
...
# Make the delivery manifest repository-relative so deterministic ZIP bytes do
# not depend on a GitHub runner or local checkout path.
builder_path = Path('project_sources/agent_runtime/tools/build_openai_gpt_deployment_release.py')
builder = builder_path.read_text(encoding='utf-8')
old = '''def _copy_record(source: Path, destination: Path, delivery_root: Path) -> dict[str, Any]:
...
if builder.count(old) != 1:
    raise SystemExit(f'expected one _copy_record definition, found {builder.count(old)}')
builder = builder.replace(old, new, 1)
builder = builder.replace(
    '_copy_record(source, destination_root / name, delivery_root)',
    '_copy_record(source, destination_root / name, delivery_root, repo_root)',
)
builder = builder.replace(
    '_copy_record(source, destination_root / "Knowledge" / source.name, delivery_root)',
    '_copy_record(source, destination_root / "Knowledge" / source.name, delivery_root, repo_root)',
)
builder = builder.replace(
...
# Add a cross-checkout determinism regression so identical source trees under
# different absolute roots must produce byte-identical ZIPs.
...
new_test = '''def test_cross_checkout_root_determinism() -> None:
    ...
    try:
        ...
        manifest_a = json.loads(
            (repo_a / "project_sources/validation/out" / module.DELIVERY_ROOT_NAME / "delivery_manifest.json").read_text(
                encoding="utf-8"
            )
        )
...
# Register the new manual workflow required-surface profile.
...
# Add the primary workflow to the issue #194 modularization contract registry.
...
insert_at = next(
    (i + 1 for i, entry in enumerate(workflow_contracts) if entry.get('file') == '.github/workflows/manual-gemini-bundle-build.yml'),
    len(workflow_contracts),
)
workflow_contracts.insert(insert_at, new_contract)
contracts_path.write_text(json.dumps(contracts, indent=2) + '\n', encoding='utf-8')

# Make the manual builder discoverable in the canonical agent-runtime docs.
...
if guide_section not in guide:
    if guide_marker not in guide:
        raise SystemExit('Deployment guide insertion marker not found')
...
'@ | python -
    ...
    git add `
      ...
      .github/github_actions/workflow_modularization_contracts.json `
...
if not report.get('success') or report.get('static_parity_status') != 'pass':
    raise SystemExit(f'combined release report not clean: {report}')
zip_path = Path(report['zip_path'])
with zipfile.ZipFile(zip_path) as zf:
    names = zf.namelist()
prefix = 'OpenAI_GPT_Deployment_Packages/'
if sum(name.startswith(prefix + 'AFRICOM_DCOIR_Analyst/Knowledge/') for name in names) != 7:
    raise SystemExit('DCOIR delivery Knowledge count is not seven')
if sum(name.startswith(prefix + 'AFRICOM_USB_Reporting/Knowledge/') for name in names) != 2:
    raise SystemExit('USB delivery Knowledge count is not two')
for required in (
    prefix + 'AFRICOM_DCOIR_Analyst/GPT_Configuration.json',
    prefix + 'AFRICOM_DCOIR_Analyst/Instructions.md',
    prefix + 'AFRICOM_USB_Reporting/GPT_Configuration.json',
    prefix + 'AFRICOM_USB_Reporting/Instructions.md',
    prefix + 'delivery_manifest.json',
    prefix + 'agent_release_parity_report.json',
    prefix + 'Release_Parity_Deployment_Readback.md',
):
    if required not in names:
        raise SystemExit(f'missing delivery entry: {required}')
...
'@ | python -
    ...
    if ($tracked) {
      $tracked | Out-Host
      throw 'Validation changed tracked repository files'
    }
