...
$worktree = Join-Path $env:RUNNER_TEMP ('dcoir-pr518-v1-' + $requestedSha.Substring(0,12))
...
try {
    ...
    @{
        ...
        Out-File -LiteralPath (Join-Path $downloads 'issue517-pr518-exact-head-validation-v1-receipt.json') -Encoding utf8

    Write-Host 'Issue #517 / PR #518 exact-head deterministic validation v1 passed.'
}
finally {
