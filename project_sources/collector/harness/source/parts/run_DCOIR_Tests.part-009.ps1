...
generated coverage matrix artifacts, and package-validation-backed source evidence.
...
function Test-CoverageArtifactEvidence {
  ...
  switch ($CapabilityId) {
    ...
    'collector.process.parent_context_lolbin_tuning' {
      try {
        $repoRoot = $null
        $searchRoot = $PSScriptRoot
        while (-not [string]::IsNullOrWhiteSpace($searchRoot)) {
          $candidateValidatorPath = Join-Path $searchRoot 'project_sources/collector/tools/validate_dcoir_collector_runtime_package.py'
          if (Test-Path -LiteralPath $candidateValidatorPath) {
            $repoRoot = $searchRoot
            break
          }
        ...
        $reportsPartPaths = @(
          (Join-Path $repoRoot 'project_sources/collector/source/parts/DCOIR_Collector.02D1A_Baseline_Collection_And_Reports.ps1'),
        ...
        $validatorReportDir = Join-Path $repoRoot 'project_sources/validation/out_harness_capability_coverage'
        New-Item -ItemType Directory -Force -Path $validatorReportDir | Out-Null
        $validatorOutput = & $pythonPath $validatorPath --source-dir $repoRoot --output-dir $validatorReportDir 2>&1
        if ($LASTEXITCODE -ne 0) { return $false }
        $validatorReportPath = Join-Path $validatorReportDir 'validate_dcoir_collector_runtime_package_report.json'
        if (-not (Test-Path -LiteralPath $validatorReportPath)) { return $false }
        $validatorReport = Get-Content -LiteralPath $validatorReportPath -Raw | ConvertFrom-Json
        if ($validatorReport.success -ne $true) { return $false }
        if ([string]$validatorReport.source_dir -ne [string](Resolve-Path -LiteralPath $repoRoot).Path) { return $false }
        $parentContextPolicy = $validatorReport.checks.suspicious_process_parent_context_policy
        if ($null -eq $parentContextPolicy) { return $false }
        ...
        return $validatorText.Contains('validate_suspicious_process_parent_context_policy') `
          ...
          -and ($parentContextPolicy.convert_accepts_process_start_lookup -eq $true) `
          -and ($parentContextPolicy.parent_start_time_checked_before_name_resolution -eq $true) `
          -and ($parentContextPolicy.wmic_command_line_indicators_present -eq $true) `
          -and ($parentContextPolicy.suppresses_only_known_benign_name_only_hits -eq $true) `
          -and ($parentContextPolicy.finding_includes_parent_process_id -eq $true) `
          -and ($parentContextPolicy.finding_includes_parent_process_name -eq $true) `
          -and ($parentContextPolicy.process_inventory_reports_parent_name -eq $true) `
          -and ($parentContextPolicy.recommendations_include_parent_context -eq $true)
      } catch {
        return $false
      }
    }
...
function Add-CollectorCapabilityCoverageRows {
  ...
  $rows = @(
    ...
    @{ Id='collector.knowledge.operator_contract_alignment'; Cat='Operator documentation'; Claim='Collector operator knowledge pages accurately describe source-backed modes, quick aliases, enrichment session guardrails, output contracts, and bounded Tier 2 validation shape.'; Sources=@('knowledge/README.md','knowledge/Knowledge - Collector - Feature and Output Contract Reference.md','knowledge/Knowledge - Core - Artifact Review Guide.md','knowledge/Knowledge - Core - Tier 2 Collect Runbook.md','knowledge/Knowledge - Core - Enrichment Actions.md'); Class='partial'; Suites=@(); Steps=@(); Risk='medium'; Value='high'; Assertions=@('knowledge docs reviewed against source','documentation-quality validator checks source comments','knowledge entry point references maintained pages'); Gap='FullRegression does not prove knowledge-doc source alignment; closeability requires separate doc/readback review evidence.' },
