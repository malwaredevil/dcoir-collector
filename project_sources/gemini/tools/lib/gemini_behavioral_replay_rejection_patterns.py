...
POST_MARKER_EVIDENCE_BOUNDARY_PATTERN = re.compile(
    r"^\s+would\s+(?:exceed|go\s+beyond|outstrip)\s+(?:the\s+)?"
    r"(?:(?:currently|available|verified|known|established)\s+){0,2}"
    r"(?:facts|evidence|data|record)\b"
)
