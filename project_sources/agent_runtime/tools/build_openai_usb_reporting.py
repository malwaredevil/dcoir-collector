...
EXPECTED_PATHS = {
    ...
    'canonical_instructions_source': (
        ...
        'openai_usb_reporting/Instructions.md'
    ),
    'behavioral_cases': (
        'project_sources/agent_runtime/provider_adapters/'
        'openai_usb_reporting/Behavioral_Contract_Cases.json'
    ),
    'generated_root': (
        'project_sources/agent_runtime/generated/packages/openai_usb_reporting'
    ),
}
EXPECTED_OUTPUTS = {
...
SECTION_HEADINGS = {
    ...
    'usb_reporting_workflow': '## USB reporting workflow',
    'queries_report_preparation': '## Queries and report preparation',
...
PROVIDER_TOPOLOGY_LEAKS = (
    ...
    'sub-agent execution',
    'ordinary dcoir analyst work may be handled here',
)
...
def _validate_instructions(
    ...
    if cases.get('schema') != 'dcoir.agent_runtime.openai_usb_reporting_behavioral_cases.v1':
    ...
    positions = [text.find(heading) for heading in SECTION_HEADINGS.values()]
    ...
    for case in case_items:
        ...
        for marker in markers:
            if not isinstance(marker, str) or not marker:
                errors.append(f'{case_id} has an invalid required marker')
...
def _knowledge_files(
    ...
    if target_manifest.get('schema') != KNOWLEDGE_SCHEMA:
        errors.append('Unexpected USB knowledge target manifest schema')
    ...
    if len(projections) != EXPECTED_KNOWLEDGE_FILES:
        errors.append(
            f'Expected {EXPECTED_KNOWLEDGE_FILES} USB knowledge files, got {len(projections)}'
        )
    ...
    knowledge_root = (
        repo_root / 'project_sources/agent_runtime/generated/knowledge/openai_usb_reporting'
    ).resolve()
...
def build_package(repo_root: Path, manifest_path: Path, check: bool) -> tuple[list[str], dict[str, Any]]:
    ...
    if not isinstance(target, dict):
        errors.append('Shared source contract lacks the OpenAI USB Reporting target')
        target = {}
    for key, expected in (
        ('provider', 'openai_webui_bedrock_hosted'),
        ('runtime_model', EXPECTED_RUNTIME_MODEL),
        ('instruction_mode', 'static_instructions'),
        ('knowledge_mode', 'static_knowledge'),
    ):
        if target.get(key) != expected:
            errors.append(f'OpenAI USB Reporting {key} drift')
    ...
    if not isinstance(target_capabilities, dict) or set(target_capabilities) != set(REQUIRED_DISABLED_CAPABILITIES):
    ...
    for capability in REQUIRED_DISABLED_CAPABILITIES:
        ...
        if target_capabilities.get(capability) is not False:
    ...
    if knowledge_projection_manifest.get('schema') != 'dcoir.agent_runtime.knowledge_projection.v1':
        errors.append('Unexpected knowledge projection manifest schema')
    knowledge_target_config = knowledge_projection_manifest.get('targets', {}).get(TARGET_ID)
    if not isinstance(knowledge_target_config, dict):
        errors.append('Knowledge projection manifest lacks the OpenAI USB target')
    else:
        if knowledge_target_config.get('target_manifest_path') != manifest.get('knowledge_target_manifest'):
            errors.append('Knowledge target manifest path disagrees across contracts')
        if knowledge_target_config.get('expected_projection_count') != EXPECTED_KNOWLEDGE_FILES:
    ...
    package_source_keys = ('canonical_instructions_source', 'behavioral_cases')
    for key in package_source_keys:
    ...
    package_manifest = {
        'schema': 'dcoir.agent_runtime.openai_usb_reporting_package.v1',
    ...
    if check:
        ...
        with tempfile.TemporaryDirectory(prefix='.openai-usb-', dir=repo_root) as temp_dir:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=(
            'project_sources/agent_runtime/provider_adapters/'
            'openai_usb_reporting/Adapter_Manifest.json'
        ),
    )
    ...
    errors, report = build_package(repo_root, manifest_path.resolve(), check=not args.materialize)
