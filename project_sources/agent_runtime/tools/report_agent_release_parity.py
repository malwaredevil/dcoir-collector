...
def _target_check_results(repo_root: Path, run_checks: bool) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for check_id, argv in TARGET_CHECKS:
        ...
        if not run_checks:
            results.append({'id': check_id, 'command': 'python ' + ' '.join(argv), 'result': 'not_run'})
            continue
        try:
            completed = subprocess.run(
                command,
        ...
        results.append({'id': check_id, 'command': 'python ' + ' '.join(argv), 'result': result})
...
def _canonical_behavior_counts(source_manifest: dict[str, Any], errors: list[str]) -> dict[str, int]:
    ...
    for item in items:
        ...
        for target_id in EXPECTED_TARGET_IDS:
            if target_id in applies_to:
                counts[target_id] += 1
    return counts


def _governed_differences(source_targets: list[dict[str, Any]]) -> list[dict[str, Any]]:
    by_id = {target.get('id'): target for target in source_targets if isinstance(target, dict)}
    differences: list[dict[str, Any]] = []
    dimensions = ('provider', 'runtime_model', 'instruction_mode', 'knowledge_mode')
...
def _validate_guide(repo_root: Path, errors: list[str]) -> dict[str, Any]:
    ...
    for marker in missing:
        errors.append(f'Manual deployment/readback guide missing required marker: {marker}')
    return {
...
def _openai_target(
    ...
    if package_manifest.get('target_id') != target_id:
        errors.append(
            f'{target_id} package manifest target_id mismatch: '
            f"{package_manifest.get('target_id')!r}"
        )
    actual_behavior = package_manifest.get('behavior_coverage_count')
    if actual_behavior != expected_behavior_count:
        errors.append(
            f'{target_id} behavior coverage mismatch: expected {expected_behavior_count}, '
            f'got {actual_behavior!r}'
        )
    ...
    if actual_knowledge != expected_knowledge:
        errors.append(
            f'{target_id} knowledge file count mismatch: expected {expected_knowledge!r}, '
            f'got {actual_knowledge!r}'
        )
    ...
    manual_required = package_manifest.get('manual_deployment_required', True) is True
    deployment_state = (
        'live_readback_recorded'
        if live_validated
    ...
    return {
        ...
        'manual_deployment_required': manual_required,
        'live_webui_validation_performed': live_validated,
        'deployment_readback_state': deployment_state,
    }
...
def _gemini_target(
    ...
    if not isinstance(modules, list):
        ...
        modules = []
    actual_behavior = len(modules)
    if actual_behavior != expected_behavior_count:
        errors.append(
            'gemini_dcoir_agent behavior module mismatch: '
            f'expected {expected_behavior_count}, got {actual_behavior}'
        )
    ...
    if len(attachments) != expected_knowledge:
        errors.append(
            'gemini_dcoir_agent knowledge attachment count mismatch: '
            f'expected {expected_knowledge!r}, got {len(attachments)}'
        )
    required_files = gemini_manifest.get('required_files')
    if not isinstance(required_files, list):
        errors.append('Gemini required_files must be an array')
        required_files = []
    projection_files: list[dict[str, Any]] = []
    for item in modules:
    ...
    return {
        ...
        'behavior': {
            'expected_canonical_items': expected_behavior_count,
            'reported_module_count': actual_behavior,
        },
        'knowledge': {
            'expected_file_count': expected_knowledge,
        ...
        'generated_files': projection_files,
        'generated_file_count': len(projection_files),
...
def build_release_report(
    ...
    *,
    ...
    target_ids = source_manifest.get('target_ids')
    if target_ids != list(EXPECTED_TARGET_IDS):
        errors.append(
            'Shared source target_ids must exactly equal the governed release order '
            f'{list(EXPECTED_TARGET_IDS)!r}; got {target_ids!r}'
        )
    ...
    if not isinstance(source_targets, list):
        ...
        source_targets = []
    source_targets_by_id = {
        item.get('id'): item for item in source_targets if isinstance(item, dict)
    }
    if set(source_targets_by_id) != set(EXPECTED_TARGET_IDS):
        errors.append(
            'Shared source targets disagree with governed target ids: '
            f'{sorted(source_targets_by_id)}'
        )
    ...
    if set(knowledge_targets) != set(EXPECTED_TARGET_IDS):
        errors.append(
            'Knowledge projection target ids disagree with governed target ids: '
            f'{sorted(knowledge_targets)}'
        )
    for target_id, target in knowledge_targets.items():
        ...
        if manifest_path:
            _resolve_repo_path(
                ...
                manifest_path,
                errors,
                f'{target_id} projection target manifest',
            )
    ...
    targets.append(
        _gemini_target(
            source_targets_by_id.get('gemini_dcoir_agent', {}),
            behavior_manifest,
            gemini_manifest,
            gemini_identity,
            knowledge_targets.get('gemini_dcoir_agent', {}),
            behavior_counts['gemini_dcoir_agent'],
            errors,
        )
    )

    openai_identities: dict[str, dict[str, Any]] = {}
    openai_manifests: dict[str, dict[str, Any]] = {}
    for target_id in ('openai_dcoir_analyst', 'openai_usb_reporting'):
        manifest, identity = _manifest_identity(
            repo_root,
            OPENAI_PACKAGE_MANIFESTS[target_id],
            errors,
            'adapter_contract_version',
        )
        openai_manifests[target_id] = manifest
        openai_identities[target_id] = identity
        targets.append(
            _openai_target(
                target_id,
                source_targets_by_id.get(target_id, {}),
                manifest,
                identity,
                knowledge_targets.get(target_id, {}),
                behavior_counts[target_id],
                errors,
            )
        )
    ...
    accepted_differences = _governed_differences(source_targets)
    blocking_gaps = [
    ...
    report = {
        ...
        'targets': targets,
        'governed_provider_differences': accepted_differences,
        ...
        'reverse_reconciliation_required': source_manifest.get(
            'generated_artifact_policy', {}
        ).get('reverse_reconciliation_required') is True,
    }
    if report['reverse_reconciliation_required'] is not True:
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for name, identity in report['source_contracts'].items():
        lines.append(
            f"| {name} | {identity.get('version') or '-'} | "
            f"`{identity.get('sha256') or 'missing'}` |"
        )
    ...
    if report['governed_provider_differences']:
        for item in report['governed_provider_differences']:
            lines.append(
                f"- `{item['dimension']}`: {json.dumps(item['values'], sort_keys=True)}"
            )
    else:
        lines.append('- None.')
    lines.extend(['', '## Blocking static parity gaps', ''])
    if report['blocking_parity_gaps']:
        for item in report['blocking_parity_gaps']:
            lines.append(f"- {item['message']}")
    else:
        lines.append('- None.')
    lines.extend(['', '## Pending live/manual evidence', ''])
    if report['pending_live_evidence']:
        for item in report['pending_live_evidence']:
            lines.append(f"- `{item['target_id']}`: `{item['state']}`")
    else:
        lines.append('- None.')
    lines.extend(['', '## Existing target checks', '', '| Check | Result |', '| --- | --- |'])
    for item in report['target_check_results']:
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = {
        ...
        'json_report': json_path.relative_to(repo_root).as_posix() if json_path.is_relative_to(repo_root) else json_path.as_posix(),
        'markdown_report': markdown_path.relative_to(repo_root).as_posix() if markdown_path.is_relative_to(repo_root) else markdown_path.as_posix(),
        'errors': errors,
    }
