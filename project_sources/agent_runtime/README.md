...
Later issues may consider Gemini runtime knowledge consolidation only after the OpenAI packages and unified release report establish a reviewed parity baseline, and may separately align remaining Supabase `ircore` routing, scenarios, source references, and validation records. The seven retired `project_sources/PP-*` references remain documented as historical drift evidence, but the live Gemini manifest now points to the shared source and module contracts.
