...
`provider_adapters/openai_usb_reporting/Adapter_Manifest.json` maps every behavior item applicable to `openai_usb_reporting` to the canonical static Instructions. Its pinned behavior-source snapshot makes any shared Prime or specialist change fail closed until the USB Reporting adapter is deliberately reviewed and reconciled. The generated package under `generated/packages/openai_usb_reporting/` contains the direct-paste Instructions, GPT editor configuration, one ordered Knowledge path with hash, and a package manifest.
