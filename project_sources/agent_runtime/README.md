...
The direct artifact expands the production ZIP so the operator can open one download and find `AFRICOM_DCOIR_Analyst/` and `AFRICOM_USB_Reporting/` beneath `OpenAI_GPT_Deployment_Packages/`. Each target folder contains one human-facing `GPT_WebUI_Setup.md`, `GPT_Configuration.json`, `manifest.json`, and a `Knowledge/` folder containing exactly the governed seven or two files. `GPT_WebUI_Setup.md` is deterministically generated from the JSON configuration, exact internal generated Instructions, and ordered Knowledge list; it is the normal operator copy/paste surface for Name/Title, Description, Instructions, conversation starters, model/runtime, capability states, and Knowledge filenames. The machine-readable JSON and manifest remain evidence surfaces. The package root also carries the deployment/readback guide, release/parity JSON and Markdown, and delivery manifests.

The direct builder enforces a governed Description ceiling of 300 characters and an Instructions ceiling of 8,000 characters, records the actual counts, re-reads the machine inputs to prove the setup Markdown regenerates byte-for-byte, and fails closed if the target root exposes any additional human-facing Markdown. The internal generated OpenAI package retains `Instructions.md` as a build/validation source; the direct operator target folder intentionally does not expose a second Instructions Markdown file.

The workflow fails closed on source/package drift, Knowledge hash/count drift, blocking static parity gaps, source-commit mismatch, field-limit overflow, setup/JSON synchronization drift, unsafe output paths, or release-builder self-test failure. Its successful output is still static deployment material only: it does not create or modify either hosted GPT and does not prove live GPT-5.4 behavior. Manual WebUI deployment and live readback remain governed by `docs/Release_Parity_Deployment_Readback.md`.
...
Gemini runtime Knowledge consolidation was evaluated by #416 after the OpenAI packages and unified release report established the parity baseline; the current decision is DEFER and the active Gemini target remains on 28 direct canonical attachments. Stage 8 issue #418 aligns Supabase `ircore` routing, scenarios, source references, tool/workflow catalogs, retrieval profiles, and validation records with this cross-provider architecture; see `docs/IrCore_Operational_Alignment.md` for the operational authority boundary and readback contract. The seven retired `project_sources/PP-*` references remain documented as historical drift evidence, but the live Gemini manifest now points to the shared source and module contracts.
