...
Each target folder exposes exactly one human-facing setup document, `GPT_WebUI_Setup.md`. That file is generated from the same validated `GPT_Configuration.json`, exact generated Instructions, and ordered Knowledge list used by the machine-readable package. It contains the exact Name/Title, Description, Instructions, conversation starters, model/runtime, capability states, and Knowledge filenames to apply in the GPT editor. `GPT_Configuration.json` and `manifest.json` remain machine-readable evidence; they are not the normal copy/paste surface.

The direct package builder fails closed if the Description exceeds the governed 300-character package ceiling, if Instructions exceed the 8,000-character hard package ceiling, if the generated setup Markdown cannot be regenerated exactly from the current JSON/Instructions/Knowledge inputs, or if more than one human-facing Markdown file appears at a target-folder root. The internal generated OpenAI package may retain `Instructions.md` as a build/validation surface, but the direct operator target folder does not expose a second Instructions Markdown file.
...
Use only the `AFRICOM_DCOIR_Analyst/` folder from the validated direct delivery artifact and the seven files under its `Knowledge/` folder.

1. Confirm the approved commit and a passing static release/parity report.
2. Run `python project_sources/agent_runtime/tools/build_openai_dcoir_analyst.py --check` immediately before deployment when working from a repository checkout; the one-click workflow already performs this check for its direct artifact.
3. Open `AFRICOM_DCOIR_Analyst/GPT_WebUI_Setup.md`. Confirm its field-limit readback reports Description at or below 300 characters and Instructions at or below 8,000 characters.
4. Open the company OpenAI/ChatGPT WebUI and edit the existing `AFRICOM DCOIR Analyst` GPT. Do not create a replacement target unless the operator explicitly intends a new GPT.
5. Work down `GPT_WebUI_Setup.md` from top to bottom: copy the Name/Title, Description, complete Instructions, conversation starters, and model/runtime exactly; apply the listed capability states exactly unless a separately governed capability change supersedes the package.
6. Remove stale Knowledge attachments and upload exactly the seven ordered filenames listed by `GPT_WebUI_Setup.md` from that target's `Knowledge/` folder. Do not upload maintainer-only Gemini material.
7. Save/update the GPT.
8. Perform the live readback steps below before claiming deployment or live parity complete.

Do not hand-edit the generated setup wording during the normal release lane. If a direct WebUI hotfix is necessary, use the reverse-reconciliation procedure below.

## AFRICOM USB Reporting manual deployment

Use only the `AFRICOM_USB_Reporting/` folder from the validated direct delivery artifact and the two files under its `Knowledge/` folder.

1. Confirm the approved commit and a passing static release/parity report.
2. Run `python project_sources/agent_runtime/tools/build_openai_usb_reporting.py --check` immediately before deployment when working from a repository checkout; the one-click workflow already performs this check for its direct artifact.
3. Open `AFRICOM_USB_Reporting/GPT_WebUI_Setup.md`. Confirm its field-limit readback reports Description at or below 300 characters and Instructions at or below 8,000 characters.
4. Open the company OpenAI/ChatGPT WebUI and edit the existing `AFRICOM USB Reporting` GPT. Do not convert the general DCOIR Analyst GPT into the USB target.
5. Work down `GPT_WebUI_Setup.md` from top to bottom: copy the Name/Title, Description, complete Instructions, conversation starters, and model/runtime exactly; apply the listed capability states exactly unless a separately governed capability change supersedes the package.
6. Remove stale Knowledge attachments and upload exactly the two ordered filenames listed by `GPT_WebUI_Setup.md` from that target's `Knowledge/` folder.
7. Save/update the GPT.
8. Perform the live readback steps below before claiming deployment or live parity complete.
...
4. Name, Description, conversation starters, and capability-toggle state compared with `GPT_WebUI_Setup.md`; use `GPT_Configuration.json` as the machine-readable cross-check if needed.
5. Full Instructions readback compared with the Instructions block in `GPT_WebUI_Setup.md`. If the WebUI permits copying the full text back out, compare exact text or a locally computed SHA-256. If it does not, record that limitation and perform a complete visual/text review.
6. Knowledge attachment filenames and count compared with the setup document and package manifest: seven for DCOIR Analyst and two for USB Reporting.
